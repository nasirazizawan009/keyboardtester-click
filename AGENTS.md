...
> **🤝 MULTI-AGENT COLLABORATION:** This project is worked on by multiple AI agents (Claude Code + Codex).
...
## Recent Update (v17.2.40 — April 25, 2026)

### Wave-6 Tools (5 new, all localized + indexed)
After 50/50 roadmap completion, scouted competitors (TestUFO, OnlineMicTest, EIZO) and shipped 5 specialized tools picked by (search volume × low competition):

- `monitor-sharpness-test.php` — Lagom-style 1px grid + RGB sub-pixel ruler (Pexels hero 6424591)
- `pitch-detector.php` — Web Audio autocorrelation Hz/note/cents (Pexels hero 26280300)
- `webcam-mirror.php` — virtual mirror with snapshot (Pexels hero 28919123)
- `hearing-age-test.php` — mosquito tone 8-22 kHz screening (Pexels hero 3394664)
- `online-ruler.php` — credit-card-calibrated SVG ruler (Pexels hero 19585984)

All 5 EN tools + 40 localized wrappers (8 langs) + schema/FAQs + tool-list-data + sitemap + IndexNow + Google Indexing API.

### Wave-3 hero image fixes
Wave-3 tools (frequency-response, accelerometer, gyroscope, vibration, gamertag) shipped reusing OLD shared hero paths and the localized wrappers referenced `images/{slug}/hero.jpg` which didn't exist → broken images on localized variants. Fixed with 5 unique fresh Pexels heroes:
- frequency-response-test → 8447625
- accelerometer-test → 11601280
- gyroscope-test → 12514724
- vibration-test → 6006298
- gamertag-generator → 30324913

### Tool-list locale bug fix (CRITICAL pattern)
`includes/components/tools-list.php` was hardcoded `renderToolsList('en')` — every localized page showed English tool names. Fixed to detect `$siteChromeLocale` set by header.php. Added 144 missing translations (18 wave-3/4/5/6 tools × 8 languages) to `includes/components/tool-list-data.php`. **All 91 tools now have full localization in 8 languages.**

### Header/sidebar locale propagation
- `includes/components/site-chrome.php` line 525: header "Tools" link now picks `pages/all-tools-{locale}.php` instead of hardcoded EN
- `includes/components/site-sidebar.php`: `kbtRenderSiteSidebar()` accepts `$locale` and resolves each tool URL via `tool['routes']['localized']` when on a localized page
- `header.php` passes `$siteChromeLocale` to the sidebar

### Dark theme contrast fix
`includes/render-all-tools.php` `.at-section` used undefined `--surface-2` CSS var → fell back to `#fafbfc` (light cream) on dark mode. Added explicit dark-theme overrides for `.at-section`, `.at-card`, `.at-cat-count`, `.at-card-title`, `.at-card-desc`.

### LCP optimization (homepage hero)
The hero YouTube thumbnail was the LCP element but had `loading="lazy"`. Changed to `loading="eager"` + `fetchpriority="high"`, wrapped in `<picture>` with WebP source. Saved ~3.4s of resource-load delay. Files updated: `help/brief-keyboard-tester.php` + 8 `languages/{lang}/sections/hero.php`. Generated 5 WebP versions of yt-thumbs (17-38% size savings each).

## Deploy Pipeline (April 25, 2026 onward)

### SFTP via SSH key (PRIMARY METHOD)
**FTPS is unreliable on this host** — server's 226-ack on TLS data channel hangs, leaving stuck connections that hit the 8-connection cap. Switched to SSH/SFTP.

- **SSH host:** `keyboardtester.click`
- **SSH port:** `19199` (non-standard)
- **SSH user:** `keyboar1` (cPanel main account, NOT the FTP user `nasir@keyboardtester.click`)
- **Auth:** publickey only (no password)
- **Private key:** `c:/xampp/htdocs/kbt/.ssh-deploy/kbt_deploy` (ed25519, generated 2026-04-25)
- **Public key:** authorized via cPanel SSH Access → Manage SSH Keys → kbt-deploy
  - **Note:** cPanel's UI authorization sometimes doesn't write `~/.ssh/authorized_keys` correctly. The working path was: cPanel File Manager → `/home2/keyboar1/.ssh/authorized_keys` (manually pasted public key). Re-authorizing via cPanel UI alone is unreliable.
- **Deploy script template:** `deploy-wave6-sftp.py` — uses paramiko, takes ~76s for 74 files
- **Remote root:** `/home2/keyboar1/public_html`

### Active deploy scripts
| Script | Purpose |
|--------|---------|
| `deploy-wave6-sftp.py` | Wave-6 SFTP deploy (template for future waves) |
| `deploy-wave5.py`, `deploy-wave4.py`, `deploy-wave3.py` | Older waves (FTPS — keep for reference, fallback only) |

### Quick paramiko deploy snippet (one-off uploads)
```python
import paramiko
client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect('keyboardtester.click', port=19199, username='keyboar1',
               ...
               allow_agent=False, look_for_keys=False)
sftp = client.open_sftp()
sftp.put(local, '/home2/keyboar1/public_html/' + remote)
sftp.close(); client.close()
```

## Bing Webmaster Tools API (April 25, 2026)
Set up bulk URL submission via official BWT API (vs UI's 10/day cap, API allows 10,000/day, 60,000/month).

- **API key:** stored in `.env` as `BING_API_KEY=...`
- **Submitter script:** `submit-bing-wmt.py` (handles sitemap → bulk submit + quota check)
- **Audit script:** `scripts/audit-bing-indexing.py` (uses `GetUrlInfo` to check crawled status)
- **Endpoints used:**
  - `POST SubmitUrlbatch` (up to 500 URLs/call)
  - `GET GetUrlSubmissionQuota`
  - `GET GetUrlInfo` (per-URL audit)

### Common commands
```bash
# Verify quota
python submit-bing-wmt.py --check
# Submit all sitemap URLs (882 currently)
python submit-bing-wmt.py
# Submit specific URLs
python submit-bing-wmt.py --urls https://keyboardtester.click/new-tool.php
# Audit recent priority URLs
python scripts/audit-bing-indexing.py
```

### Findings (April 25, 2026)
- All 207 priority URLs (recent waves' EN + 8 localized) confirmed crawled by Bing (`DiscoveryDate` set)
- Indexing-for-ranking is a separate question — Bing's SERP visibility is gated by domain authority + backlinks, not crawl status
- Real lever for Bing visibility: external backlinks (see backlink section below)

## Backlink Strategy (April 25, 2026)

### Internal linking
- **Matrix file:** `scripts/blog-cross-link-matrix.json` (24 blog posts × 23 recent tools)
- **Builder:** `scripts/build-link-matrix.py` (regenerate matrix)
- **Applier:** `scripts/apply-link-matrix.py` (idempotent — adds new `<li>` entries to "Related Tools" sections in blog posts)
- **Status:** 15 cross-links added across 12 blog posts on April 25, 2026

### External backlinks — GitHub awesome-list PRs
**Tracking file:** `seo-audit-2026-04-23/awesome-list-prs.md` — full list of 21 submissions with URLs, status, and acceptance probability.

**Submission scripts:**
- `scripts/submit-awesome-prs.py` — broad lists (no-login, online-tools, esports, etc.)
- `scripts/submit-specialized-prs.py` — topic-specific lists (privacy, music, a11y, design, etc.)
- `scripts/retry-failed-prs.py` — retry helper for SHA mismatch / non-standard READMEs

**Auth:** Uses GitHub PAT with `public_repo` scope (passed via `GITHUB_TOKEN` env var). User generates a 7-day fine-scoped token, pastes it, scripts run, then user revokes. Don't store the token in any file.

**Submitted on April 25, 2026 (21 total):**
- 8 broad-list PRs (aviaryan/awesome-no-login-web-apps #411, moyi4681/awesome-mechanical-keyboard #1, fiatjaf/awesome-loginless #44, brandonhimpfen/awesome-audio-engineering #1, amilajack/awesome-web-audio #114, janstk/Awesome-online-tools #19, StanForever/awesome-websites #49, mathewlewallen/awesome-free-tools #62)
- 13 specialized-list PRs (ad-si/awesome-music-production #217, olilarkin/awesome-musicdsp #12, pluja/awesome-privacy #789, iAnonymous3000/awesome-privacy-tools #18, Delapouite/awesome-keyboard #8, esteves-esta/awesome-touch-typing #3, EanNewton/Awesome-Keebs #3, Strift/awesome-esports #5, K3V1991/Awesome-Gaming-List #5, brunopulis/awesome-a11y #226, devtoolsd/awesome-devtools #206, goabstract/Awesome-Design-Tools #520, Siddharth11/Colorful #26)
- 1 issue (Keycapsss/awesome-mechanical-keyboard #201 — asks where Keebfolio-format repo wants tool entries)

**Skipped:** DolbyIO/awesome-audio (archived), t18n/awesome-dev-tools (archived), Lissy93/awesome-privacy + paulaime/awesome-privacy (auto-generated READMEs from data files — Contents API can't write these), KevinColemanInc/awesome-privacy + ryanmagoon/awesome-a11y + moimikey/awesome-devtools (non-standard README paths or fork sync issues).

**Expected:** 6-10 will merge over 2-4 weeks. Each merge typically yields 1 direct + 5-30 indirect backlinks (via list forks/mirrors like awesomerank.github.io).

## Site State Snapshot (April 25, 2026)
- **91 tools registered** in `includes/components/tool-list-data.php`
- **Roadmap: 50/50 complete** (`.Codex/tool-roadmap.md`); wave-6 was post-roadmap competitive scouting
- **Sitemap:** 882 URLs (5 wave-6 EN + 40 wave-6 localized + earlier waves' localized + intent pages + blog)
- **Localization:** 8 languages (es, fr, de, pt, ar, ru, ja, ko) — all 91 tools translated in `tool-list-data.php` locales section
- **Tools-list locale rendering:** WORKS on localized pages (post-fix)
- **Header + sidebar locale rendering:** WORKS — links route to localized variants when present
- **Deploy:** SFTP/paramiko (FTPS is broken)
- **BWT bulk submit:** automated via `submit-bing-wmt.py` + `.env` API key
- **Google Indexing API:** still works via `submit-google-indexing.py` + `google-credentials.json`
- **IndexNow:** still works via `submit-indexnow.php?key=kbt-submit-2024`

## Tooling Reminders for Future Sessions
- **Always use SFTP/paramiko, not FTPS** — see Deploy Pipeline above. FTPS will hang on the post-STOR 226 ack.
- **Always set `BING_API_KEY` in `.env`**, then run `python submit-bing-wmt.py` after any new sitemap update
- **Always run `python scripts/gen-localized-wrappers.py`** when adding a new tool (generates 8 localized wrappers)
- **Always update `tool-list-data.php` with all 8 locale translations** when adding a new tool — otherwise localized pages show English names in their tool list. The pattern in the file: each tool gets an entry under each `'es' => [...]`, `'fr' => [...]`, etc. block's `'tools' => [...]` map.
- **Pexels hero IDs already in use** — see AGENTS.md history (33+ unique IDs). Always check uniqueness before downloading new ones.
- **GitHub token for PR submission:** never persist; user generates 7-day fine-scoped PAT, pastes once, script uses, user revokes.
