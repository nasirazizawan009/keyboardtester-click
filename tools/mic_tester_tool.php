<?php
// Microphone Tester tool (standalone)
?>
...
<div class="kbt-tool kbt-mic-tool" id="mic-tool">
  <div class="kbt-tool-grid two">
    <div class="kbt-tool-card">
      ...
      <p>Allow microphone access to visualize live audio levels. No audio is recorded or uploaded.</p>
      <div class="kbt-tool-actions">
        <button class="kbt-tool-button primary" id="mic-start">Start mic</button>
        <button class="kbt-tool-button" id="mic-stop" disabled>Stop</button>
      </div>
      <div class="kbt-tool-status" id="mic-status">Status: Idle</div>
    </div>
    <div class="kbt-tool-card">
      <h3>Level meter</h3>
      ...
      <div class="kbt-tool-stats">
        <div class="kbt-tool-stat"><span>Current level</span><strong id="mic-level">0%</strong></div>
        <div class="kbt-tool-stat"><span>Peak</span><strong id="mic-peak">0%</strong></div>
      </div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function startMic() {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      statusEl.textContent = 'Status: Microphone not supported';
      return;
    }
    try {
