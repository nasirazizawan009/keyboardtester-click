<?php
// QR Code Reader tool (standalone)
?>
...
<div class="kbt-tool kbt-qr-reader-tool" id="qr-reader-tool">
  <div class="kbt-tool-grid two">
    <div class="kbt-tool-card">
      ...
      <p>Select an image with a QR code. We decode it locally in your browser.</p>
      <input class="kbt-tool-input" type="file" id="qr-reader-file" accept="image/*">
      <div class="kbt-tool-actions">
        <button class="kbt-tool-button primary" id="qr-reader-run" disabled>Decode</button>
        <button class="kbt-tool-button ghost" id="qr-reader-clear">Clear</button>
      </div>
      <div class="kbt-tool-status" id="qr-reader-status">Status: Waiting for image</div>
      <div class="qr-output" id="qr-reader-preview">Image preview</div>
    </div>
    <div class="kbt-tool-card">
      <h3>Decoded result</h3>
      <textarea class="kbt-tool-textarea" id="qr-reader-output" rows="10" placeholder="QR code content appears here..." readonly></textarea>
      <div class="kbt-tool-actions">
        <button class="kbt-tool-button ghost" id="qr-reader-copy">Copy</button>
      </div>
    </div>
  </div>
</div>
  ...
  runBtn.addEventListener('click', function () {
    ...
    img.onload = function () {
      ...
      if (code) {
        ...
        statusEl.textContent = 'Status: QR code decoded';
      } else {
        output.value = 'No QR code detected in this image.';
        statusEl.textContent = 'Status: No QR code found';
      }
    };
    img.onerror = function () {
      statusEl.textContent = 'Status: Unable to read image';
    };
