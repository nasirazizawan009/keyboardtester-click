<?php
// webcamtestertool.php — embeddable Webcam Tester module (no <html> shell)
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="wc-grid">
      ...
      <div class="wc-card">
        ...
        <div class="wc-video-container" id="wc-videoContainer">
          <div class="wc-placeholder" id="wc-placeholder">
            ...
            <p style="margin: 0; font-size: 16px; font-weight: 600;">Webcam Preview</p>
            <p style="margin: 8px 0 0; font-size: 13px;">Click "Start Webcam" to begin testing</p>
          </div>
          ...
          <div class="wc-overlay" id="wc-overlay" style="display: none;">
            ...
            <div class="wc-badge" id="wc-resolutionBadge">Loading...</div>
          </div>
        </div>

        <div class="wc-controls">
          <button id="wc-startBtn" class="wc-btn wc-btn-primary">Start Webcam</button>
          <button id="wc-stopBtn" class="wc-btn wc-btn-danger" disabled>Stop Webcam</button>
          <button id="wc-snapshotBtn" class="wc-btn wc-btn-accent" disabled>Take Snapshot</button>
          <button id="wc-downloadBtn" class="wc-btn wc-btn-ghost" disabled>Download All</button>
        </div>

        <!-- Snapshots Section -->
        <div id="wc-snapshotsSection" style="display: none;">
          <h4 style="margin: 20px 0 0; font-size: 14px; color: var(--muted);">Captured Snapshots (<span id="wc-snapshotCount">0</span>)</h4>
    ...
    <div>
      ...
      <div class="wc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Webcam Information</h3>
        <div class="wc-info-grid">
          <div class="wc-info-item">
            <span class="wc-info-label">Status</span>
            <span class="wc-info-value" id="wc-infoStatus">Not Started</span>
          </div>
          <div class="wc-info-item">
          ...
          <div class="wc-info-item">
            <span class="wc-info-label">Aspect Ratio</span>
            <span class="wc-info-value" id="wc-infoAspect">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Frame Rate</span>
            <span class="wc-info-value" id="wc-infoFPS">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Device</span>
      ...
      <!-- Settings -->
      <div class="wc-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Settings</h3>
        <div class="wc-settings">
          <div class="wc-setting-item">
            <label for="wc-cameraSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Select Camera</label>
          ...
          <div class="wc-setting-item">
            <label for="wc-resolutionSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Preferred Resolution</label>
        ...
        <div style="margin-top: 20px;">
          <h4 style="margin: 0 0 12px; font-size: 14px; color: var(--muted);">Features</h4>
          <div class="wc-feature-list">
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Real-time video preview</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Multiple camera support</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Snapshot & download</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Resolution detection</span>
            </div>
          </div>
        </div>
      </div>
...
(()=>{
  ...
  async function startWebcam() {
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        showInfo('Webcam started successfully!');
      };
      
    } catch (err) {
      console.error('Error accessing webcam:', err);
      showError('Could not access webcam. Please check permissions and try again.');
    }
  }
  ...
  function updateInfo(reset = false) {
    if (reset) {
      document.getElementById('wc-infoStatus').textContent = 'Not Started';
    ...
    document.getElementById('wc-infoDevice').textContent = videoTrack.label || 'Default Camera';
  ...
  function downloadAll() {
    snapshots.forEach((snapshot, index) => {
      ...
      link.download = `webcam-snapshot-${index + 1}.png`;
      link.click();
    });
  }
