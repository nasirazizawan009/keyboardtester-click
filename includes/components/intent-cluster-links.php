...
require_once __DIR__ . '/../seo-clusters.php';

$clusterKey = $intentClusterTool ?? $currentTool ?? null;
$cluster = $clusterKey ? getSeoIntentCluster($clusterKey) : null;
$currentPath = basename(parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH) ?: ($_SERVER['PHP_SELF'] ?? ''));

if (!$cluster || empty($cluster['pages'])) {
    return;
}

$clusterPages = array_values(array_filter($cluster['pages'], static function ($page) use ($currentPath) {
    return basename($page['path']) !== $currentPath;
}));
        ...
        <div class="intent-cluster-grid">
            <?php foreach ($clusterPages as $page): ?>
                <a class="intent-cluster-card" href="<?php echo url($page['path']); ?>">
                    ...
                    <span class="intent-cluster-link">Open page</span>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
