<?php
/**
 * Portuguese Keyboard Tester - Full Layout Tool
 * Portuguese QWERTY layout with cedilla and accent keys
 */
...
<section class="keyboard-tester" id="keyboard-tester">
    <div class="controls-wrapper">
        <div class="textarea-wrapper">
            <div class="textarea-label">Historico de Teclas</div>
            <textarea class="text-box" id="key-history" rows="4" placeholder="Pressione qualquer tecla para comecar o teste..."></textarea>
        </div>
    ...
    <div class="feature-controls" id="feature-controls" style="display: none;">
        ...
        <button class="feature-button" id="sound-button">
            ...
            <span class="sound-label">Som: Desativado</span>
        </button>
    ...
    <div class="stats-panel" id="stats-panel" style="display: none;">
        <div class="stats-header">
            <h3><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 20V10M12 20V4M6 20v-6"/></svg> Estatisticas da Sessao</h3>
            <button class="close-btn" id="close-stats">&times;</button>
        </div>
        <div class="stats-content">
            <div class="stat-item"><span class="stat-label">Total de Pressionamentos</span><span class="stat-value" id="total-keys">0</span></div>
            <div class="stat-item"><span class="stat-label">Duracao da Sessao</span><span class="stat-value" id="session-time">00:00</span></div>
            <div class="stat-item"><span class="stat-label">Teclas Testadas</span><span class="stat-value" id="keys-tested">0/104</span></div>
            <div class="stat-item"><span class="stat-label">Mais Pressionada</span><span class="stat-value" id="most-pressed">Nenhuma</span></div>
        </div>
    </div>

    <div class="stats-panel" id="latency-panel" style="display: none;">
        <div class="stats-header">
            <h3><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> Teste de Latencia</h3>
        ...
        <div class="stats-content">
            <div class="stat-item latency-display">
                ...
                <div class="latency-sublabel">Latencia Atual</div>
            </div>
            <div class="latency-grid">
                <div class="stat-item compact"><span class="stat-value mono" id="avg-latency">--</span><span class="stat-label">Media</span></div>
                <div class="stat-item compact"><span class="stat-value mono" id="min-latency">--</span><span class="stat-label">Melhor</span></div>
                <div class="stat-item compact"><span class="stat-value mono" id="max-latency">--</span><span class="stat-label">Pior</span></div>
            </div>
        </div>
    </div>
        ...
        <div class="keyboard-layout">
            <div class="keyboard-section main-keyboard">
                ...
                <div class="key-row">
                    ...
                    <button class="key key-2x" data-key="Backspace"><span>Retrocesso</span></button>
                </div>
                <div class="key-row">
            ...
            <div class="keyboard-section nav-section">
                <div class="nav-top">
                    ...
                    <div class="key-row">
                        ...
                        <button class="key" data-key="End"><span>Fim</span></button>
...
.key-1-25x { width: 60px; } .key-1-5x { width: 72px; } .key-1-75x { width: 84px; } .key-2x { width: 96px; } .key-2-25x { width: 108px; } .key-enter { width: 114px; } .key-shift-right { width: 120px; } .key-space { width: 336px; }
...
(function() {
    ...
    const catProgress = (catProgressRoot && window.KeyboardCatProgress)
        ? new window.KeyboardCatProgress(catProgressRoot, { desktopTotalKeys: TOTAL_KEYS, mobileTotalKeys: 42, locale: 'pt' })
        : null;
    ...
    if (soundButton) soundButton.addEventListener('click', () => { state.soundEnabled = !state.soundEnabled; soundButton.classList.toggle('active', state.soundEnabled); soundButton.querySelector('.sound-label').textContent = state.soundEnabled ? 'Som: Ativado' : 'Som: Desativado'; });
...
function handleKeyDown(e) {
    ...
    function reset() { state.keyPressCount = {}; state.uniqueKeys.clear(); state.totalPresses = 0; state.sessionStart = null; state.latencyData = []; state.lastKeyTime = 0; if (state.sessionTimer) { clearInterval(state.sessionTimer); state.sessionTimer = null; } if ($('key-history')) $('key-history').value = ''; if ($('total-keys')) $('total-keys').textContent = '0'; if ($('session-time')) $('session-time').textContent = '00:00'; if ($('keys-tested')) $('keys-tested').textContent = `0/${TOTAL_KEYS}`; if ($('most-pressed')) $('most-pressed').textContent = 'Nenhuma'; if ($('current-latency')) $('current-latency').textContent = '-- ms'; if ($('avg-latency')) $('avg-latency').textContent = '--'; if ($('min-latency')) $('min-latency').textContent = '--'; if ($('max-latency')) $('max-latency').textContent = '--'; $$('.key').forEach(k => { k.classList.remove('pressed', 'active-1', 'active-2', 'active-3', 'active-4', 'active-5'); const counter = k.querySelector('.key-counter'); if (counter) counter.remove(); }); $$('.indicator').forEach(i => i.classList.remove('active')); resetCatProgress(); }
...
if (!isset($GLOBALS['__kbt_adv_loaded'])) {
    ...
    echo "<script>window.KBT_LOCALE = 'pt';</script>";
