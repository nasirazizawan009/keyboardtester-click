<?php
// Latency Checker tool (standalone) - Portuguese Version
?>

<div class="kbt-tool kbt-latency-tool" id="latency-tool">
  <div class="kbt-tool-grid two">
    <div class="kbt-tool-card">
      <h3>Teste de latencia</h3>
      <p>Pressione qualquer tecla enquanto o teste estiver ativo. Estimamos o atraso de processamento para cada pressionamento.</p>
      <div class="kbt-tool-actions">
        <button class="kbt-tool-button primary" id="latency-start">Iniciar teste</button>
        <button class="kbt-tool-button" id="latency-stop" disabled>Parar</button>
        <button class="kbt-tool-button ghost" id="latency-clear">Limpar</button>
      </div>
      <div class="kbt-tool-status" id="latency-status">Status: Pronto</div>
    </div>
    <div class="kbt-tool-card">
      <h3>Resultados</h3>
      <div class="kbt-tool-stats">
        <div class="kbt-tool-stat"><span>Atual</span><strong id="latency-current">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Media</span><strong id="latency-avg">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Melhor</span><strong id="latency-min">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Pior</span><strong id="latency-max">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Amostras</span><strong id="latency-count">0</strong></div>
      </div>
      <div class="kbt-tool-log" id="latency-log">Pressione Iniciar e depois pressione teclas para capturar amostras.</div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  startBtn.addEventListener('click', function () {
    active = true;
    statusEl.textContent = 'Status: Testando (pressione teclas)';
  ...
  clearBtn.addEventListener('click', function () {
    ...
    logEl.textContent = 'Pressione Iniciar e depois pressione teclas para capturar amostras.';
  });
