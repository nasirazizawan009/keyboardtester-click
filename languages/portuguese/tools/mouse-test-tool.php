<?php
// Portuguese Mouse Tester Tool - Testador de Mouse
?>
<div class="mouse-tester">
  <canvas id="mouse-canvas" width="400" height="250" role="button" aria-label="Area de teste do mouse, clique em qualquer botao ou role"></canvas>

  <div class="results">
    <div class="result-box" id="left-click-count" aria-live="polite">Clique Esquerdo: 0</div>
    <div class="result-box" id="middle-click-count" aria-live="polite">Clique do Meio: 0</div>
    <div class="result-box" id="right-click-count" aria-live="polite">Clique Direito: 0</div>
    <div class="result-box" id="scroll-count" aria-live="polite">Rolagem: 0</div>
  </div>

  <div class="status">
    <div class="status-box" id="left-click-status">Esquerdo: Liberado (Ultimo: Nenhum)</div>
    <div class="status-box" id="middle-click-status">Meio: Liberado (Ultimo: Nenhum)</div>
    <div class="status-box" id="right-click-status">Direito: Liberado (Ultimo: Nenhum)</div>
    <div class="status-box" id="scroll-status">Rolagem: Nenhuma (Ultimo: Nenhum)</div>
  </div>

  <button class="reset-button" id="reset-button" aria-label="Reiniciar todos os contadores e status">Reiniciar</button>
</div>
...
<script>
  ...
  const buttonStates = {
    left: { status: 'Liberado', lastPress: 'Nenhum' },
    middle: { status: 'Liberado', lastPress: 'Nenhum' },
    right: { status: 'Liberado', lastPress: 'Nenhum' },
    scroll: { status: 'Nenhuma', lastPress: 'Nenhum' }
  };
  ...
  function updateStatus(button, status, timestamp) {
    ...
    if (timestamp) buttonStates[button].lastPress = new Date().toLocaleTimeString('pt-BR');
    const buttonNames = { left: 'Esquerdo', middle: 'Meio', right: 'Direito', scroll: 'Rolagem' };
    const displayText = button === 'scroll' ?
      `Rolagem: ${buttonStates[button].status} (Ultimo: ${buttonStates[button].lastPress})` :
    ...
    switch (button) {
      case 'left':
        ...
        leftClickStatus.classList.toggle('left-click-pressed', status === 'Pressionado');
        break;
      case 'middle':
        middleClickStatus.textContent = displayText;
        middleClickStatus.classList.toggle('middle-click-pressed', status === 'Pressionado');
        break;
      case 'right':
        rightClickStatus.textContent = displayText;
        rightClickStatus.classList.toggle('right-click-pressed', status === 'Pressionado');
        break;
      case 'scroll':
        scrollStatus.textContent = displayText;
        scrollStatus.classList.toggle('scroll-pressed', status !== 'Nenhuma');
        break;
    }
  }
  ...
  canvas.addEventListener('mousedown', (event) => {
    ...
    if (buttonType) {
      ...
      switch (buttonType) {
        case 'left':
          leftClickCount++;
          leftClickDisplay.textContent = `Clique Esquerdo: ${leftClickCount}`;
          leftClickDisplay.classList.add('left-click-active');
          updateStatus('left', 'Pressionado', true);
          break;
        case 'middle':
          middleClickCount++;
          middleClickDisplay.textContent = `Clique do Meio: ${middleClickCount}`;
          middleClickDisplay.classList.add('middle-click-active');
          updateStatus('middle', 'Pressionado', true);
          break;
        case 'right':
          rightClickCount++;
          rightClickDisplay.textContent = `Clique Direito: ${rightClickCount}`;
          rightClickDisplay.classList.add('right-click-active');
          updateStatus('right', 'Pressionado', true);
          break;
      }
    }
  });
  ...
  resetButton.addEventListener('click', () => {
    ...
    scrollOffset = 0;
    lastScrollDirection = null;
    buttonStates.left = { status: 'Liberado', lastPress: 'Nenhum' };
    buttonStates.middle = { status: 'Liberado', lastPress: 'Nenhum' };
    buttonStates.right = { status: 'Liberado', lastPress: 'Nenhum' };
    buttonStates.scroll = { status: 'Nenhuma', lastPress: 'Nenhum' };
    leftClickDisplay.textContent = 'Clique Esquerdo: 0';
    middleClickDisplay.textContent = 'Clique do Meio: 0';
    rightClickDisplay.textContent = 'Clique Direito: 0';
    scrollDisplay.textContent = 'Rolagem: 0';
    leftClickStatus.textContent = 'Esquerdo: Liberado (Ultimo: Nenhum)';
    middleClickStatus.textContent = 'Meio: Liberado (Ultimo: Nenhum)';
    rightClickStatus.textContent = 'Direito: Liberado (Ultimo: Nenhum)';
    scrollStatus.textContent = 'Rolagem: Nenhuma (Ultimo: Nenhum)';
