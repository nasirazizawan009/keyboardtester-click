<?php
// screen-test-tool.php — embeddable Screen Test module (Portuguese version)
?>
<style>
  :root {
    --bg: #f6f8fc;
    ...
    <div>
      <div class="st-card">
        <h2 style="margin: 0 0 16px 0; font-size: 20px; color: var(--text); display: flex; align-items: center; gap: 10px;">
          ...
          Pre-visualizacao do Teste de Tela
        </h2>
        ...
        <div class="st-preview-container" id="st-preview-box" onclick="startScreenTest()">
          <div class="st-preview" id="st-preview-display" style="background: #000;">
            Clique para iniciar o teste em tela cheia
          </div>
          <div class="st-overlay">
            <div class="st-badge">
              <span id="st-current-color">Pronto</span>
            </div>
            <div class="st-badge">
              Clique em qualquer lugar para testar
            </div>
          </div>
        </div>

        <div class="st-colors-grid">
          <button class="st-color-btn" style="background: #000;" onclick="setPreviewColor('#000000', 'Preto')" title="Preto - Melhor para encontrar pixels mortos">
            Preto
          </button>
          <button class="st-color-btn" style="background: #fff; color: #000; text-shadow: none;" onclick="setPreviewColor('#FFFFFF', 'Branco')" title="Branco - Melhor para encontrar pixels travados">
            Branco
          </button>
          <button class="st-color-btn" style="background: #ff0000;" onclick="setPreviewColor('#FF0000', 'Vermelho')" title="Vermelho - Verificar subpixels vermelhos">
            Vermelho
          </button>
          <button class="st-color-btn" style="background: #00ff00;" onclick="setPreviewColor('#00FF00', 'Verde')" title="Verde - Verificar subpixels verdes">
            Verde
          </button>
          <button class="st-color-btn" style="background: #0000ff;" onclick="setPreviewColor('#0000FF', 'Azul')" title="Azul - Verificar subpixels azuis">
            Azul
          </button>
          <button class="st-color-btn" style="background: #ffff00; color: #000; text-shadow: none;" onclick="setPreviewColor('#FFFF00', 'Amarelo')" title="Amarelo - Subpixels V+V">
            Amarelo
          </button>
          <button class="st-color-btn" style="background: #ff00ff;" onclick="setPreviewColor('#FF00FF', 'Magenta')" title="Magenta - Subpixels V+A">
            Magenta
          </button>
          <button class="st-color-btn" style="background: #00ffff; color: #000; text-shadow: none;" onclick="setPreviewColor('#00FFFF', 'Ciano')" title="Ciano - Subpixels V+A">
            Ciano
          </button>
        </div>

        <div class="st-alert st-alert-info">
          <strong>💡 Dica:</strong> Limpe sua tela primeiro! Use as setas do teclado (← →) ou clique para alternar entre cores no modo tela cheia. Pressione ESC para sair.
        </div>
      </div>
    </div>
    ...
    <div>
      <div class="st-card">
        ...
        <div class="st-stats-grid">
          ...
          <div class="st-stat-card">
            <div class="label">Cores Testadas</div>
        ...
        <div class="st-section">
          ...
          <ul class="st-tips-list">
            <li><strong>Pixels Mortos:</strong> Pontos pretos permanentes (em todas as cores)</li>
            <li><strong>Pixels Travados:</strong> Pontos fixos em uma unica cor (vermelho/verde/azul)</li>
            <li><strong>Pixels Quentes:</strong> Pixels sempre brancos</li>
            <li><strong>Uniformidade de Cor:</strong> Cor uniforme em toda a tela</li>
          </ul>
        </div>
        ...
        <div class="st-section">
          <h3>
            <span class="st-icon">✅</span>
            Melhores Praticas de Teste
          </h3>
          <ul class="st-tips-list">
            <li>Limpe sua tela com um pano de microfibra</li>
            <li>Teste em um ambiente escuro ou com pouca luz</li>
            <li>Olhe da sua distancia normal de uso</li>
            <li>Teste todas as 8 cores completamente</li>
            <li>Verifique cuidadosamente as bordas e cantos</li>
            <li>Nao tenha pressa - leve seu tempo</li>
          </ul>
        </div>

        <div class="st-alert st-alert-success">
          <strong>🔒 Privacidade Primeiro:</strong> Este teste roda inteiramente no seu navegador. Nenhum dado e coletado ou enviado para qualquer servidor.
        </div>
      </div>
    </div>
  </div>
...
<div class="st-fullscreen-test" id="st-fullscreen-test">
  <div class="st-fs-instructions" id="st-fs-instructions">
    <div style="font-weight: 600; margin-bottom: 8px;">Teste de Tela Ativo</div>
    <div style="font-size: 12px;">Use as setas do teclado (← →) ou clique para mudar cores • Pressione ESC para sair</div>
  </div>
  <div class="st-fs-controls">
    <button class="st-fs-btn" onclick="previousColor()">← Anterior</button>
    <button class="st-fs-btn" onclick="nextColor()">Proximo →</button>
    <button class="st-fs-btn" onclick="exitFullScreen()">Sair (ESC)</button>
  </div>
</div>

<script>
  const colors = [
    { hex: '#000000', name: 'Preto', desc: 'Procure pixels mortos (pontos pretos)' },
    { hex: '#FFFFFF', name: 'Branco', desc: 'Procure pixels travados (pontos coloridos)' },
    { hex: '#FF0000', name: 'Vermelho', desc: 'Verifique subpixels vermelhos' },
    { hex: '#00FF00', name: 'Verde', desc: 'Verifique subpixels verdes' },
    { hex: '#0000FF', name: 'Azul', desc: 'Verifique subpixels azuis' },
    { hex: '#FFFF00', name: 'Amarelo', desc: 'Subpixels Vermelho + Verde' },
    { hex: '#FF00FF', name: 'Magenta', desc: 'Subpixels Vermelho + Azul' },
    { hex: '#00FFFF', name: 'Ciano', desc: 'Subpixels Verde + Azul' }
  ];
