<?php
// headphone-test-tool.php - modulo de teste de fones/alto-falantes integravel (sem shell <html>)
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="hs-grid">
    ...
    <div>
      <div class="hs-card">
        <div id="hs-infoAlert" class="hs-alert hs-alert-info">
          <strong>Dica:</strong> Use fones de ouvido para a melhor experiencia de teste estereo. Ajuste o volume do seu sistema para um nivel confortavel antes de comecar.
        </div>
        ...
        <div class="hs-visualizer" id="hs-visualizer">
          <div class="hs-placeholder" id="hs-placeholder">
            ...
            <p style="margin: 8px 0 0; font-size: 13px;">Selecione um teste abaixo para comecar</p>
          </div>
          ...
          <div class="hs-overlay" id="hs-overlay" style="display: none;">
            <div class="hs-badge">
              ...
              <span id="hs-statusText">Reproduzindo</span>
            </div>
        ...
        <div class="hs-controls">
          <button id="hs-stopBtn" class="hs-btn hs-btn-danger" disabled>Parar Audio</button>
        </div>
    ...
    <div>
      ...
      <div class="hs-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Teste Rapido de Canais</h3>
        <div class="hs-channel-grid">
          <button class="hs-channel-btn" id="hs-leftBtn">
            <div style="font-size: 24px; margin-bottom: 8px;">⬅️</div>
            <div>Canal Esquerdo</div>
          </button>
          <button class="hs-channel-btn" id="hs-rightBtn">
            <div style="font-size: 24px; margin-bottom: 8px;">➡️</div>
            <div>Canal Direito</div>
          </button>
        </div>
        <button class="hs-channel-btn" id="hs-stereoBtn" style="margin-top: 12px; width: 100%;">
          <div style="font-size: 24px; margin-bottom: 8px;">🔊</div>
          <div>Ambos os Canais (Estereo)</div>
        </button>
      </div>

      <!-- Audio Tests -->
      <div class="hs-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Testes de Audio</h3>
        <div class="hs-test-grid">
          <div class="hs-test-item" data-test="sweep">
            <div class="hs-test-title">🎼 Varredura de Frequencia</div>
            <div class="hs-test-desc">Varredura de 20Hz a 20kHz para testar toda a faixa de audio</div>
          </div>
          <div class="hs-test-item" data-test="bass">
            <div class="hs-test-title">🔉 Teste de Graves (100Hz)</div>
            <div class="hs-test-desc">Teste a resposta de baixa frequencia e o subwoofer</div>
          </div>
          <div class="hs-test-item" data-test="mid">
            <div class="hs-test-title">🔊 Faixa Media (1kHz)</div>
            <div class="hs-test-desc">Teste a clareza de vozes e instrumentos</div>
          </div>
          <div class="hs-test-item" data-test="treble">
            <div class="hs-test-title">🔔 Teste de Agudos (10kHz)</div>
            <div class="hs-test-desc">Teste a resposta e clareza de alta frequencia</div>
          </div>
          <div class="hs-test-item" data-test="balance">
            <div class="hs-test-title">⚖️ Teste de Balanco</div>
            <div class="hs-test-desc">Alterna entre os canais esquerdo e direito</div>
          </div>
          <div class="hs-test-item" data-test="noise">
            <div class="hs-test-title">📻 Ruido Branco</div>
            <div class="hs-test-desc">Ruido de espectro completo para teste de alto-falantes</div>
          </div>
        </div>
      </div>
      ...
      <div class="hs-card" style="margin-top: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Informacoes de Audio</h3>
        <div class="hs-info-grid">
          <div class="hs-info-item">
            <span class="hs-info-label">Status</span>
            <span class="hs-info-value" id="hs-infoStatus">Pronto</span>
          </div>
          <div class="hs-info-item">
            <span class="hs-info-label">Teste Atual</span>
            <span class="hs-info-value" id="hs-infoTest">Nenhum</span>
          </div>
          <div class="hs-info-item">
            <span class="hs-info-label">Frequencia</span>
...
(()=>{
  ...
  function playTone(frequency, panValue = 0, channelName = 'Estereo') {
    ...
    updateInfo(false, `Tom de ${frequency}Hz`, frequency, channelName);
  }
  ...
  function playNoise() {
    ...
    updateInfo(false, 'Ruido Branco', 'Espectro Completo', 'Estereo');
  }
  ...
  function playSweep() {
    ...
    updateInfo(false, 'Varredura de Frequencia', '20Hz - 20kHz', 'Estereo');
  }
  ...
  function playBalance() {
    ...
    const balanceInterval = setInterval(() => {
      ...
      const channelName = panValue === -1 ? 'Esquerdo' : 'Direito';
    ...
    updateInfo(false, 'Teste de Balanco', '440Hz', 'Alternando');
  }
  ...
  function updateInfo(reset = false, testName = '', frequency = '', channel = '') {
    if (reset) {
      infoStatus.textContent = 'Pronto';
      infoStatus.style.color = 'var(--muted)';
      infoTest.textContent = 'Nenhum';
    ...
    infoStatus.textContent = 'Reproduzindo';
  ...
  leftBtn.addEventListener('click', function() {
    playTone(440, -1, 'Apenas Esquerdo');
  ...
  stereoBtn.addEventListener('click', function() {
    playTone(440, 0, 'Ambos os Canais');
    this.classList.add('playing');
    leftBtn.classList.remove('playing');
    rightBtn.classList.remove('playing');
  });
