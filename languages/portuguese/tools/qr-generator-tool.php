<?php
// Portuguese QR Code Generator tool - Gerador de Codigo QR
?>

<div class="kbt-tool kbt-qr-tool" id="qr-generator-tool">
  <div class="kbt-tool-grid two">
    <div class="kbt-tool-card">
      <h3>Gerar codigo QR</h3>
      <div class="kbt-tool-field">
        <label for="qr-text">Texto ou URL</label>
        <textarea class="kbt-tool-textarea" id="qr-text" rows="4" placeholder="Insira texto ou URL"></textarea>
      </div>
      <div class="kbt-tool-field">
      ...
      <div class="kbt-tool-actions">
        <button class="kbt-tool-button primary" id="qr-generate">Gerar</button>
        <button class="kbt-tool-button ghost" id="qr-download">Baixar</button>
      </div>
    </div>
    <div class="kbt-tool-card">
      <h3>Pre-visualizacao</h3>
      <div class="qr-output" id="qr-output">Pre-visualizacao do codigo QR</div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderQR() {
    ...
    if (!text) {
      output.textContent = 'Pre-visualizacao do codigo QR';
      return;
    }
