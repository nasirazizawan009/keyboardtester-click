<?php
// webcam-test-tool.php — embeddable Webcam Tester module (Portuguese version)
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="wc-grid">
      ...
      <div class="wc-card">
        ...
        <div class="wc-video-container" id="wc-videoContainer">
          <div class="wc-placeholder" id="wc-placeholder">
            ...
            <p style="margin: 0; font-size: 16px; font-weight: 600;">Pre-visualizacao da Webcam</p>
            <p style="margin: 8px 0 0; font-size: 13px;">Clique em "Iniciar Webcam" para comecar o teste</p>
          </div>
          ...
          <div class="wc-overlay" id="wc-overlay" style="display: none;">
            <div class="wc-badge">
              ...
              <span id="wc-statusText">Ao Vivo</span>
            </div>
            <div class="wc-badge" id="wc-resolutionBadge">Carregando...</div>
          </div>
        </div>

        <div class="wc-controls">
          <button id="wc-startBtn" class="wc-btn wc-btn-primary">Iniciar Webcam</button>
          <button id="wc-stopBtn" class="wc-btn wc-btn-danger" disabled>Parar Webcam</button>
          <button id="wc-snapshotBtn" class="wc-btn wc-btn-accent" disabled>Tirar Foto</button>
          <button id="wc-downloadBtn" class="wc-btn wc-btn-ghost" disabled>Baixar Tudo</button>
        </div>

        <!-- Snapshots Section -->
        <div id="wc-snapshotsSection" style="display: none;">
          <h4 style="margin: 20px 0 0; font-size: 14px; color: var(--muted);">Fotos Salvas (<span id="wc-snapshotCount">0</span>)</h4>
    ...
    <div>
      ...
      <div class="wc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Informacoes da Webcam</h3>
        <div class="wc-info-grid">
          <div class="wc-info-item">
            <span class="wc-info-label">Status</span>
            <span class="wc-info-value" id="wc-infoStatus">Nao Iniciada</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Resolucao</span>
            <span class="wc-info-value" id="wc-infoResolution">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Proporcao</span>
            <span class="wc-info-value" id="wc-infoAspect">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Taxa de Quadros</span>
      ...
      <!-- Settings -->
      <div class="wc-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Configuracoes</h3>
        <div class="wc-settings">
          <div class="wc-setting-item">
            <label for="wc-cameraSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Selecionar Camera</label>
          ...
          <div class="wc-setting-item">
            <label for="wc-resolutionSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Resolucao Preferida</label>
            <select id="wc-resolutionSelect" class="wc-select">
              <option value="default">Automatico (Padrao)</option>
        ...
        <div style="margin-top: 20px;">
          <h4 style="margin: 0 0 12px; font-size: 14px; color: var(--muted);">Recursos</h4>
          <div class="wc-feature-list">
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Pre-visualizacao de video em tempo real</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Suporte para multiplas cameras</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Captura e download de fotos</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Deteccao de resolucao</span>
            </div>
          </div>
        </div>
      </div>
...
(()=>{
  ...
  async function getCameras() {
    try {
      ...
      cameraSelect.innerHTML = '<option value="">Erro ao carregar cameras</option>';
    }
  }
  ...
  async function startWebcam() {
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        showInfo('Webcam iniciada com sucesso!');
      };

    } catch (err) {
      console.error('Error accessing webcam:', err);
      showError('Nao foi possivel acessar a webcam. Por favor, verifique as permissoes e tente novamente.');
    }
  }
  ...
  function updateInfo(reset = false) {
    if (reset) {
      document.getElementById('wc-infoStatus').textContent = 'Nao Iniciada';
    ...
    document.getElementById('wc-infoStatus').textContent = 'Ativa';
    ...
    document.getElementById('wc-infoDevice').textContent = videoTrack.label || 'Camera Padrao';
  ...
  function downloadAll() {
    snapshots.forEach((snapshot, index) => {
      ...
      link.download = `foto-webcam-${index + 1}.png`;
      link.click();
    });
  }
