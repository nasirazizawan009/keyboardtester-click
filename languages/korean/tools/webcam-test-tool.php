<?php
// Korean Webcam Tester tool - 웹캠 테스터
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="wc-grid">
      ...
      <div class="wc-card">
        ...
        <div class="wc-video-container" id="wc-videoContainer">
          <div class="wc-placeholder" id="wc-placeholder">
            ...
            <p style="margin: 0; font-size: 16px; font-weight: 600;">웹캠 미리보기</p>
            <p style="margin: 8px 0 0; font-size: 13px;">"웹캠 시작"을 클릭하여 테스트를 시작하세요</p>
          </div>
          ...
          <div class="wc-overlay" id="wc-overlay" style="display: none;">
            <div class="wc-badge">
              ...
              <span id="wc-statusText">실시간</span>
            </div>
            <div class="wc-badge" id="wc-resolutionBadge">로딩 중...</div>
          </div>
        </div>

        <div class="wc-controls">
          <button id="wc-startBtn" class="wc-btn wc-btn-primary">웹캠 시작</button>
          <button id="wc-stopBtn" class="wc-btn wc-btn-danger" disabled>웹캠 중지</button>
          <button id="wc-snapshotBtn" class="wc-btn wc-btn-accent" disabled>스냅샷 촬영</button>
          <button id="wc-downloadBtn" class="wc-btn wc-btn-ghost" disabled>전체 다운로드</button>
        </div>

        <!-- Snapshots Section -->
        <div id="wc-snapshotsSection" style="display: none;">
          <h4 style="margin: 20px 0 0; font-size: 14px; color: var(--muted);">캡처된 스냅샷 (<span id="wc-snapshotCount">0</span>)</h4>
    ...
    <div>
      ...
      <div class="wc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">웹캠 정보</h3>
        <div class="wc-info-grid">
          <div class="wc-info-item">
            <span class="wc-info-label">상태</span>
            <span class="wc-info-value" id="wc-infoStatus">시작 전</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">해상도</span>
            <span class="wc-info-value" id="wc-infoResolution">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">화면 비율</span>
            <span class="wc-info-value" id="wc-infoAspect">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">프레임 레이트</span>
            <span class="wc-info-value" id="wc-infoFPS">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">장치</span>
      ...
      <!-- Settings -->
      <div class="wc-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">설정</h3>
        <div class="wc-settings">
          <div class="wc-setting-item">
            <label for="wc-cameraSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">카메라 선택</label>
          ...
          <div class="wc-setting-item">
            <label for="wc-resolutionSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">선호 해상도</label>
            <select id="wc-resolutionSelect" class="wc-select">
              <option value="default">자동 (기본값)</option>
        ...
        <div style="margin-top: 20px;">
          <h4 style="margin: 0 0 12px; font-size: 14px; color: var(--muted);">기능</h4>
          <div class="wc-feature-list">
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">실시간 비디오 미리보기</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">다중 카메라 지원</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">스냅샷 및 다운로드</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">해상도 감지</span>
            </div>
          </div>
        </div>
      </div>
...
(()=>{
  ...
  async function getCameras() {
    try {
      ...
      cameraSelect.innerHTML = '<option value="">카메라 로딩 오류</option>';
    }
  }
  ...
  async function startWebcam() {
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        showInfo('웹캠이 성공적으로 시작되었습니다!');
      };

    } catch (err) {
      console.error('Error accessing webcam:', err);
      showError('웹캠에 접근할 수 없습니다. 권한을 확인하고 다시 시도하세요.');
    }
  }
  ...
  function updateInfo(reset = false) {
    if (reset) {
      document.getElementById('wc-infoStatus').textContent = '시작 전';
    ...
    document.getElementById('wc-infoStatus').textContent = '활성';
    ...
    document.getElementById('wc-infoDevice').textContent = videoTrack.label || '기본 카메라';
  ...
  function downloadAll() {
    snapshots.forEach((snapshot, index) => {
      ...
      link.download = `웹캠-스냅샷-${index + 1}.png`;
      link.click();
    });
  }
  ...
  navigator.mediaDevices.getUserMedia({ video: true })
    ...
    .catch(() => {
      // Permission denied, cameras will show as "카메라 1", "카메라 2", etc.
    });
})();
</script>
