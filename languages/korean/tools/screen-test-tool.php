<?php
// Korean Screen Test tool - 화면 테스터
?>
<style>
  :root {
    --bg: #f6f8fc;
      ...
      <div class="st-card">
        ...
        <div class="st-preview-container" id="st-preview-box" onclick="startScreenTest()">
          ...
          <div class="st-overlay">
            <div class="st-badge">
              <span id="st-current-color">준비됨</span>
            </div>
            <div class="st-badge">
              아무 곳이나 클릭하여 테스트
            </div>
        ...
        <div class="st-colors-grid">
          <button class="st-color-btn" style="background: #000;" onclick="setPreviewColor('#000000', '검정')" title="검정 - 데드 픽셀 찾기에 최적">
            검정
          </button>
          <button class="st-color-btn" style="background: #fff; color: #000; text-shadow: none;" onclick="setPreviewColor('#FFFFFF', '흰색')" title="흰색 - 스턱 픽셀 찾기에 최적">
            흰색
          </button>
          <button class="st-color-btn" style="background: #ff0000;" onclick="setPreviewColor('#FF0000', '빨강')" title="빨강 - 빨간 서브픽셀 확인">
            빨강
          </button>
          <button class="st-color-btn" style="background: #00ff00;" onclick="setPreviewColor('#00FF00', '초록')" title="초록 - 초록 서브픽셀 확인">
            초록
          </button>
          <button class="st-color-btn" style="background: #0000ff;" onclick="setPreviewColor('#0000FF', '파랑')" title="파랑 - 파란 서브픽셀 확인">
            파랑
          </button>
          <button class="st-color-btn" style="background: #ffff00; color: #000; text-shadow: none;" onclick="setPreviewColor('#FFFF00', '노랑')" title="노랑 - 빨강+초록 서브픽셀">
            노랑
          </button>
          <button class="st-color-btn" style="background: #ff00ff;" onclick="setPreviewColor('#FF00FF', '마젠타')" title="마젠타 - 빨강+파랑 서브픽셀">
            마젠타
          </button>
          <button class="st-color-btn" style="background: #00ffff; color: #000; text-shadow: none;" onclick="setPreviewColor('#00FFFF', '시안')" title="시안 - 초록+파랑 서브픽셀">
            시안
          </button>
        </div>

        <div class="st-alert st-alert-info">
          <strong>💡 팁:</strong> 먼저 화면을 닦으세요! 전체 화면 모드에서 화살표 키(← →) 또는 클릭으로 색상을 전환하세요. ESC를 눌러 종료합니다.
        </div>
      </div>
    </div>
      ...
      <div class="st-card">
        ...
        <div class="st-stats-grid">
          <div class="st-stat-card">
            <div class="label">테스트 횟수</div>
            <div class="value" id="st-test-count">0</div>
          </div>
          <div class="st-stat-card">
            <div class="label">테스트한 색상</div>
        ...
        <div class="st-section">
          ...
          <ul class="st-tips-list">
            <li><strong>데드 픽셀:</strong> 영구적인 검은 점 (모든 색상에서)</li>
            <li><strong>스턱 픽셀:</strong> 한 색상에 고정된 점 (빨강/초록/파랑)</li>
            <li><strong>핫 픽셀:</strong> 항상 흰색인 픽셀</li>
            <li><strong>색상 균일성:</strong> 전체 화면에서 균일한 색상</li>
          </ul>
        </div>
        ...
        <div class="st-section">
          ...
          <ul class="st-tips-list">
            <li>극세사 천으로 화면을 닦으세요</li>
            <li>어둡거나 조명이 약한 방에서 테스트하세요</li>
            <li>평소 사용하는 거리에서 보세요</li>
            <li>8가지 색상을 모두 철저히 테스트하세요</li>
            <li>가장자리와 모서리를 주의 깊게 살펴보세요</li>
            <li>서두르지 마세요 - 충분한 시간을 가지세요</li>
          </ul>
        </div>

        <div class="st-alert st-alert-success">
          <strong>🔒 개인정보 보호:</strong> 이 테스트는 전적으로 브라우저에서 실행됩니다. 데이터가 수집되거나 서버로 전송되지 않습니다.
        </div>
      </div>
    </div>
  </div>
...
<div class="st-fullscreen-test" id="st-fullscreen-test">
  <div class="st-fs-instructions" id="st-fs-instructions">
    <div style="font-weight: 600; margin-bottom: 8px;">화면 테스트 활성화</div>
    <div style="font-size: 12px;">화살표 키(← →) 또는 클릭으로 색상 전환 • ESC를 눌러 종료</div>
  </div>
  <div class="st-fs-controls">
    <button class="st-fs-btn" onclick="previousColor()">← 이전</button>
    <button class="st-fs-btn" onclick="nextColor()">다음 →</button>
    <button class="st-fs-btn" onclick="exitFullScreen()">종료 (ESC)</button>
  </div>
</div>

<script>
  const colors = [
    { hex: '#000000', name: '검정', desc: '데드 픽셀(검은 점)을 찾으세요' },
    { hex: '#FFFFFF', name: '흰색', desc: '스턱 픽셀(색상 점)을 찾으세요' },
    { hex: '#FF0000', name: '빨강', desc: '빨간 서브픽셀 확인' },
    { hex: '#00FF00', name: '초록', desc: '초록 서브픽셀 확인' },
    { hex: '#0000FF', name: '파랑', desc: '파란 서브픽셀 확인' },
    { hex: '#FFFF00', name: '노랑', desc: '빨강 + 초록 서브픽셀' },
    { hex: '#FF00FF', name: '마젠타', desc: '빨강 + 파랑 서브픽셀' },
    { hex: '#00FFFF', name: '시안', desc: '초록 + 파랑 서브픽셀' }
  ];
