...
<div class="click-tester-section">
    <div class="click-tester">
        ...
        <div class="info-area">
            <div class="info-box">시간 <span id="sel-dur">5s</span></div>
            <div class="info-box">클릭 <span id="cur-clicks">—</span></div>
            <div class="info-box">남은 시간 <span id="time-left">5s</span></div>
        </div>
        ...
        <div class="result-strip" id="result-strip" style="display:none">
            ...
            <div class="result-row">
                <span class="result-label">클릭</span>
                <span class="result-val" id="res-clicks">—</span>
            </div>
            <p class="result-remark" id="res-remark"></p>
            <button class="new-test-btn" id="new-test-btn">새 테스트</button>
        </div>
        ...
        <div class="test-history" id="test-history" style="display:none">
            <div class="history-header">
                <span>테스트 기록</span>
                <button id="clear-history">지우기</button>
            </div>
            <table class="history-table">
                <thead><tr><th>#</th><th>시간</th><th>클릭</th><th>CPS</th></tr></thead>
...
(function () {
    ...
    function endTest() {
        ...
        clickArea.textContent = '테스트 완료';

        resCps.textContent    = cps;
        resClicks.textContent = `${clickCount} (${dur}초)`;
    ...
    function getRemark(cps) {
        if (cps > 14) return "나비 클릭 신의 경지. 당신은 인간인가요?";
        if (cps > 10) return "지터 클릭! 팔뚝이 반대하고 있어요.";
        if (cps > 7)  return "평균 이상 — 훌륭한 게임 속도!";
        if (cps > 4)  return "평균 범위. 아직 개선의 여지가 있어요!";
        return "천천히 가도 괜찮아요 — 손가락을 풀어봐요!";
    }
