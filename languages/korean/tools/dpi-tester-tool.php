<?php
// 마우스 DPI 테스터 v2.0 — 한국어
$dpi_strings = [
  'tab_test'=>'DPI 테스트','tab_edpi'=>'eDPI 계산기','tab_conv'=>'감도 변환기',
  'setup'=>'1 — 설정','lbl_dist'=>'마우스를 이동할 실제 거리',
  'unit_cm'=>'cm','unit_in'=>'인치','lbl_exp'=>'설정 DPI',
  'exp_hint'=>'(선택 — 오차율 및 정확도 표시)',
  'lbl_axis'=>'측정 축 제한','axis_both'=>'모든 방향 (권장)',
  'axis_x'=>'수평만 (X축)','axis_y'=>'수직만 (Y축)',
  'btn_start'=>'▶ 측정 시작','btn_reset'=>'초기화',
  'tip'=>'<strong>팁:</strong> 마우스패드에 자를 대고 시작점과 끝점을 표시한 후, 마우스 버튼을 누른 채로 측정 영역을 드래그하세요.',
  'track_lbl'=>'DPI 추적 영역','track_title'=>'2 — 추적 및 결과',
  'track_idle'=>'측정 시작 후 여기에서 드래그',
  'track_ready'=>'버튼을 누르고 측정 거리만큼 드래그',
  'stat_px'=>'이동 픽셀','stat_dpi'=>'예상 DPI','stat_dev'=>'오차율','stat_acc'=>'정확도',
  'runs_lbl'=>'측정 기록 (드래그로 추가)','run_n'=>'측정','run_avg'=>'평균:',
  'run_more'=>' / %d회 — 다시 드래그 ↔','run_done'=>'모든 측정 완료 — 위 평균 확인',
  'privacy'=>'🔒 이 테스트는 브라우저에서만 실행됩니다. 데이터는 수집되지 않습니다.',
  'edpi_setup'=>'나의 설정','lbl_dpi'=>'마우스 DPI','lbl_sens'=>'게임 내 감도',
  'lbl_game'=>'게임 (비교용)','game_any'=>'— 기타 / 사용자 설정 —',
  'edpi_lbl'=>'eDPI — 유효 DPI','edpi_formula'=>'<strong>공식:</strong> eDPI = 마우스 DPI × 게임 내 감도',
  'pro_title'=>'프로 범위 비교','pro_intro'=>'내 eDPI가 아래 표에 강조 표시됩니다.',
  'pro_h1'=>'eDPI 범위','pro_h2'=>'레벨','pro_h3'=>'주로 사용하는 플레이어',
  'conv_curr'=>'현재 설정','lbl_old_dpi'=>'현재 DPI',
  'lbl_old_s'=>'현재 게임 내 감도','conv_target'=>'목표 설정',
  'lbl_new_dpi'=>'새 DPI (마우스 변경 후)',
  'conv_sub'=>'새 감도 — 동일한 근육 기억, 다른 DPI',
  'conv_how'=>'작동 원리',
  'conv_exp1'=>'변환기는 <strong>eDPI를 일정하게</strong> 유지하여 동일한 물리적 움직임이 항상 동일한 게임 내 시점 이동을 만들어냅니다.',
  'conv_formula'=>'새 감도 = (기존 DPI × 기존 감도) / 새 DPI',
  'conv_exp2'=>'<strong>예시:</strong> 400 DPI 감도 2.0 → 800 DPI 감도 1.0. 둘 다 eDPI = 800.',
  'log_title'=>'세션 로그',
  'log_dl'=>'&#11015; 로그 다운로드 (.txt)',
  'log_empty'=>'아직 측정 기록이 없습니다. DPI 테스트를 실행하세요.',
];
?>
...
<div class="kbt-tool dpi-v2" id="dpi-tool">
  ...
  <div class="dpi-panel active" id="dpi-p-test" role="tabpanel">
    <div class="dpi-mode-row"><div class="dpi-mode-toggle"><button class="dpi-mode-btn active" data-mode="basic">기본</button><button class="dpi-mode-btn" data-mode="advanced">고급</button></div></div>
    <div class="dpi-grid">
      <div class="dpi-card">
        ...
        <div class="dpi-field"><label for="dpi-expected"><?php echo $dpi_strings['lbl_exp']; ?> <small><?php echo $dpi_strings['exp_hint']; ?></small></label><input class="dpi-input full" id="dpi-expected" type="number" min="100" max="32000" step="100" placeholder="예: 800"></div>
  ...
  <div class="dpi-panel" id="dpi-p-edpi" role="tabpanel">
    <div class="dpi-grid">
      ...
      <div class="dpi-card">
        ...
        <tbody id="edpi-tbody">
          <tr data-min="0"    data-max="400">   <td>200–400</td>  <td>초저감도</td>    <td>FPS 프로 (S1mple, sh1ro)</td></tr>
          <tr data-min="400"  data-max="800">   <td>400–800</td>  <td>저감도</td>      <td>경쟁 FPS / Valorant 프로</td></tr>
          <tr data-min="800"  data-max="1600">  <td>800–1600</td> <td>중간</td>        <td>일반 게이머, 캐주얼 FPS</td></tr>
          <tr data-min="1600" data-max="3200">  <td>1600–3200</td><td>고감도</td>      <td>MOBA / RTS</td></tr>
          <tr data-min="3200" data-max="999999"><td>3200+</td>    <td>초고감도</td>    <td>사무, 디자인</td></tr>
        </tbody></table>
      </div>
    </div>
    <p class="dpi-privacy"><?php echo $dpi_strings['privacy']; ?></p>
    ...
    <div class="dpi-grid">
      <div class="dpi-card">
        ...
        <div class="dpi-conv-result">
          <p class="dpi-conv-formula" id="conv-formula">새 감도 = (400 &times; 2.0) / 800</p>
...
function accuracy(p){if(p<=2)return{label:'\uc6b0\uc218 \u2713',cls:'s-good'};if(p<=5)return{label:'\uc591\ud638 \u2713',cls:'s-good'};if(p<=10)return{label:'\uc591\ud638',cls:'s-warn'};return{label:'\ubd88\ub9cc\uc871',cls:'s-bad'};}
...
function updConv(){var od=parseFloat(cOd.value)||0,os=parseFloat(cOs.value)||0,nd=parseFloat(cNd.value)||0;if(od&&os&&nd){cFm.innerHTML='\uc0c8 \uac10\ub3c4 = ('+od+' &times; '+os+') / '+nd;cVl.textContent=((od*os)/nd).toFixed(4);}else{cVl.textContent='--';}}
