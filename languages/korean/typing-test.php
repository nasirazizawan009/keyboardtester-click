<?php
/**
 * Korean Typing Speed Test - 타자 속도 테스트
 */
include __DIR__ . '/../../config.php';
include __DIR__ . '/config-ko.php';

$pageTitle = '타자 속도 테스트 - 무료 온라인 타이핑 속도 측정';
$pageDescription = '무료 온라인 타자 속도 테스트로 타이핑 속도와 정확도를 측정하세요. 실시간 피드백과 빠른 초기화 기능을 제공합니다.';
$pageKeywords = '타자 속도 테스트, 타이핑 테스트, WPM 테스트, 타자 연습, 타이핑 속도';
?>
<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title><?php echo $pageTitle; ?></title>
  <meta name="description" content="<?php echo $pageDescription; ?>">
  <meta name="keywords" content="<?php echo $pageKeywords; ?>">

  <link rel="alternate" hreflang="en" href="<?php echo absoluteUrl('keyboard_typing_test.php'); ?>">
  <link rel="alternate" hreflang="ko" href="<?php echo absoluteUrl('languages/korean/typing-test.php'); ?>">
  <link rel="alternate" hreflang="x-default" href="<?php echo absoluteUrl('keyboard_typing_test.php'); ?>">

  <?php include __DIR__ . '/../../includes/head-common.php'; ?>

  <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600&family=Space+Grotesk:wght@400;500;600;700&family=Noto+Sans+KR:wght@400;500;600;700&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="<?php echo url('assets/css/index-modern.css'); ?>">

  <script type="application/ld+json">
  {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "mainEntity": [
      {
        "@type": "Question",
        "name": "WPM이란 무엇인가요?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "WPM(Words Per Minute)은 분당 타이핑하는 단어 수를 나타냅니다. 일반적으로 5글자를 1단어로 계산합니다."
        }
      },
      {
        "@type": "Question",
        "name": "평균 타자 속도는 어느 정도인가요?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "평균 타자 속도는 약 40-50 WPM입니다. 60 WPM 이상이면 빠른 편이고, 100 WPM 이상이면 전문가 수준입니다."
        }
      },
      {
        "@type": "Question",
        "name": "타자 속도를 향상시키려면 어떻게 해야 하나요?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "정확도를 먼저 높이고, 꾸준히 연습하며, 올바른 손가락 위치를 유지하세요. 매일 15-30분씩 연습하면 빠르게 향상됩니다."
        }
      },
      {
        "@type": "Question",
        "name": "이 테스트는 한국어도 지원하나요?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "네, 이 테스트는 한국어 텍스트로 타자 속도를 측정할 수 있습니다. 난이도를 선택하여 다양한 한국어 문장을 연습할 수 있습니다."
        }
      }
    ]
  }
  </script>

  <style>
    body { font-family: 'Noto Sans KR', 'Space Grotesk', sans-serif; }
  </style>
</head>
<body class="landing-page">
  <?php include __DIR__ . '/header-ko.php'; ?>

  <main id="main-content" class="landing-main">
    <!-- Hero Section -->
    <section class="hero" aria-labelledby="hero-title">
      <div class="container hero-grid">
        <div class="hero-content">
          <h1 id="hero-title">타자 속도 테스트</h1>
          <p class="hero-lede">타이핑 속도와 정확도를 측정하세요. 실시간 피드백으로 빠르게 결과를 확인할 수 있습니다.</p>
          <div class="hero-ctas">
            <a href="#typing-test" class="landing-btn landing-btn-primary">테스트 시작</a>
            <a href="#guidelines" class="landing-btn landing-btn-ghost">사용 방법</a>
          </div>
        </div>
      </div>
    </section>

    <!-- Tool Section -->
    <section class="tool-stage" aria-labelledby="tool-stage-title">
      <div class="container tool-stage-header">
        <div>
          <p class="section-kicker">주요 테스터</p>
          <h2 id="tool-stage-title">타자 속도 테스트</h2>
          <p class="section-lede">아래 도구를 사용하여 타자 속도를 측정하세요.</p>
        </div>
        <div class="tool-stage-actions">
          <a class="landing-btn landing-btn-ghost" href="#guidelines">빠른 팁 보기</a>
        </div>
      </div>
      <section id="typing-test" class="tool-shell">
        <?php include __DIR__ . '/tools/typing-test-tool.php'; ?>
      </section>
    </section>

    <!-- Trust Strip -->
    <section class="trust-strip" aria-label="주요 기능">
      <div class="container trust-grid">
        <div class="trust-item">
          <div class="trust-title">실시간 피드백</div>
          <div class="trust-desc">즉시 결과 확인</div>
        </div>
        <div class="trust-item">
          <div class="trust-title">빠른 초기화</div>
          <div class="trust-desc">바로 다시 테스트</div>
        </div>
        <div class="trust-item">
          <div class="trust-title">브라우저 기반</div>
          <div class="trust-desc">설치 불필요</div>
        </div>
        <div class="trust-item">
          <div class="trust-title">타자 속도 전용</div>
          <div class="trust-desc">정밀 측정</div>
        </div>
      </div>
    </section>

    <!-- Features -->
    <section class="feature-band" aria-labelledby="feature-title">
      <div class="container">
        <div class="section-head">
          <p class="section-kicker">타자 속도</p>
          <h2 id="feature-title">타자 속도 테스트에 필요한 모든 것</h2>
          <p class="section-lede">집중적인 테스트를 실행하고 몇 초 안에 결과를 확인하세요.</p>
        </div>
        <div class="landing-feature-grid">
          <article class="landing-feature-card">
            <h3>집중 분석</h3>
            <p>실시간 업데이트로 타자 속도를 추적합니다.</p>
          </article>
          <article class="landing-feature-card">
            <h3>즉각적인 결과</h3>
            <p>테스트하는 동안 실시간으로 변화를 확인합니다.</p>
          </article>
          <article class="landing-feature-card">
            <h3>간단한 조작</h3>
            <p>몇 초 만에 시작, 중지, 초기화할 수 있습니다.</p>
          </article>
          <article class="landing-feature-card">
            <h3>반복 테스트</h3>
            <p>여러 번 테스트하여 빠르게 비교합니다.</p>
          </article>
        </div>
      </div>
    </section>

    <!-- Process Steps -->
    <section class="process-band" aria-labelledby="process-title">
      <div class="container">
        <div class="section-head split">
          <div>
            <p class="section-kicker">간단한 워크플로우</p>
            <h2 id="process-title">타자 속도를 테스트하는 세 단계</h2>
          </div>
          <p class="section-lede">아래의 간단한 단계를 따라 테스트하고 결과를 확인하세요.</p>
        </div>
        <div class="process-grid">
          <article class="process-card">
            <div class="step-number">01</div>
            <h3>테스트 시작</h3>
            <p>도구를 열고 테스트할 준비를 합니다.</p>
          </article>
          <article class="process-card">
            <div class="step-number">02</div>
            <h3>텍스트 입력</h3>
            <p>타이머가 끝날 때까지 제시된 텍스트를 입력합니다.</p>
          </article>
          <article class="process-card">
            <div class="step-number">03</div>
            <h3>결과 확인</h3>
            <p>타자 속도 통계를 확인하고 필요시 재테스트합니다.</p>
          </article>
        </div>
      </div>
    </section>

    <!-- Tools List -->
    <?php $currentTool = 'typing-test'; include __DIR__ . '/sections/tools-list-ko.php'; ?>

    <!-- Guidelines -->
    <section id="guidelines" class="guidelines-section">
      <div class="container">
        <div class="section-head">
          <p class="section-kicker">사용 가이드</p>
          <h2>타자 속도 테스트 사용 방법</h2>
        </div>
        <div class="guidelines-grid">
          <div class="guideline-card">
            <h3>1. 설정 선택</h3>
            <p>난이도(쉬움, 보통, 어려움)와 테스트 시간(30초, 1분, 2분)을 선택하세요.</p>
          </div>
          <div class="guideline-card">
            <h3>2. 테스트 시작</h3>
            <p>"테스트 시작" 버튼을 클릭하여 테스트를 시작하세요.</p>
          </div>
          <div class="guideline-card">
            <h3>3. 텍스트 입력</h3>
            <p>화면에 표시된 텍스트를 최대한 정확하고 빠르게 입력하세요.</p>
          </div>
          <div class="guideline-card">
            <h3>4. 결과 확인</h3>
            <p>테스트가 끝나면 WPM(분당 단어 수)과 정확도를 확인하세요.</p>
          </div>
        </div>
      </div>
    </section>
  </main>

  <?php include __DIR__ . '/footer-ko.php'; ?>
</body>
</html>
