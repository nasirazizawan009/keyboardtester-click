<?php
/**
 * German Keyboard Tester - Full QWERTZ Layout Tool
 * German keyboard layout with Y/Z swapped and special characters: ae, oe, ue, ss
 */
...
<section class="keyboard-tester" id="keyboard-tester">
    <div class="controls-wrapper">
        <div class="textarea-wrapper">
            <div class="textarea-label">Tastenhistorie</div>
            <textarea class="text-box" id="key-history" rows="4" placeholder="Druecken Sie eine Taste, um den Test zu starten..."></textarea>
        </div>
    ...
    <div class="feature-controls" id="feature-controls" style="display: none;">
        ...
        <button class="feature-button" id="sound-button">
            ...
            <span class="sound-label">Ton: Aus</span>
        </button>
    ...
    <div class="stats-panel" id="stats-panel" style="display: none;">
        <div class="stats-header">
            <h3><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 20V10M12 20V4M6 20v-6"/></svg> Sitzungsstatistiken</h3>
            <button class="close-btn" id="close-stats">&times;</button>
        </div>
        <div class="stats-content">
            <div class="stat-item"><span class="stat-label">Gesamte Tastendruecke</span><span class="stat-value" id="total-keys">0</span></div>
            <div class="stat-item"><span class="stat-label">Sitzungsdauer</span><span class="stat-value" id="session-time">00:00</span></div>
            <div class="stat-item"><span class="stat-label">Getestete Tasten</span><span class="stat-value" id="keys-tested">0/104</span></div>
            <div class="stat-item"><span class="stat-label">Meistgedrueckt</span><span class="stat-value" id="most-pressed">Keine</span></div>
        </div>
    </div>

    <div class="stats-panel" id="latency-panel" style="display: none;">
        <div class="stats-header">
            <h3><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> Latenztest</h3>
        ...
        <div class="stats-content">
            <div class="stat-item latency-display">
                ...
                <div class="latency-sublabel">Aktuelle Latenz</div>
            </div>
            <div class="latency-grid">
                <div class="stat-item compact"><span class="stat-value mono" id="avg-latency">--</span><span class="stat-label">Durchschnitt</span></div>
                <div class="stat-item compact"><span class="stat-value mono" id="min-latency">--</span><span class="stat-label">Beste</span></div>
                <div class="stat-item compact"><span class="stat-value mono" id="max-latency">--</span><span class="stat-label">Schlechteste</span></div>
            </div>
        </div>
    </div>
        ...
        <div class="keyboard-layout">
            <div class="keyboard-section main-keyboard">
                ...
                <div class="key-row">
                    ...
                    <button class="key" data-key="KeyY"><span>Z</span></button>
                ...
                <!-- QWERTZ Row 2 - Home row with Umlauts -->
                <div class="key-row">
                    <button class="key key-1-75x" data-key="CapsLock"><span>Feststell</span></button>
                ...
                <div class="key-row">
                    <button class="key key-2-25x" data-key="ShiftLeft"><span>Umschalt</span></button>
                    <button class="key" data-key="IntlBackslash"><span class="dual">&gt;<br>&lt;</span></button>
                    <button class="key" data-key="KeyZ"><span>Y</span></button>
                    ...
                    <button class="key key-shift-right" data-key="ShiftRight"><span>Umschalt</span></button>
                </div>
                <!-- Bottom Row -->
                <div class="key-row">
                    <button class="key key-1-25x" data-key="ControlLeft"><span>Strg</span></button>
                    ...
                    <button class="key key-space" data-key="Space"><span>Leertaste</span></button>
                    ...
                    <button class="key key-1-25x" data-key="ControlRight"><span>Strg</span></button>
                </div>
            </div>
            ...
            <div class="keyboard-section nav-section">
                <div class="nav-top">
                    <div class="key-row">
                        <button class="key" data-key="PrintScreen"><span>Druck</span></button>
                        <button class="key" data-key="ScrollLock"><span>Rollen</span></button>
                        <button class="key" data-key="Pause"><span>Pause</span></button>
                    </div>
                    <div class="key-row">
                        <button class="key" data-key="Insert"><span>Einfg</span></button>
                        <button class="key" data-key="Home"><span>Pos1</span></button>
                        <button class="key" data-key="PageUp"><span>Bild↑</span></button>
                    </div>
                    <div class="key-row">
                        <button class="key" data-key="Delete"><span>Entf</span></button>
                        <button class="key" data-key="End"><span>Ende</span></button>
                        <button class="key" data-key="PageDown"><span>Bild↓</span></button>
                    </div>
                </div>
            ...
            <div class="keyboard-section numpad-section">
                <div class="indicator-panel">
                    <div class="indicator" id="caps-indicator"><div class="led" id="caps-led"></div><span>Fest</span></div>
                    <div class="indicator" id="num-indicator"><div class="led" id="num-led"></div><span>Num</span></div>
                    <div class="indicator" id="scroll-indicator"><div class="led" id="scroll-led"></div><span>Roll</span></div>
                </div>
                <div class="numpad-grid">
                    ...
                    <div class="key-row">
                        ...
                        <button class="key key-tall key-enter-tall" data-key="NumpadEnter"><span>Eing</span></button>
                    </div>
                    <div class="key-row">
...
.key-1-25x { width: 60px; } .key-1-5x { width: 72px; } .key-1-75x { width: 84px; } .key-2x { width: 96px; } .key-2-25x { width: 108px; } .key-enter { width: 114px; } .key-shift-right { width: 132px; } .key-space { width: 336px; }
...
(function() {
    ...
    const catProgress = (catProgressRoot && window.KeyboardCatProgress)
        ? new window.KeyboardCatProgress(catProgressRoot, { desktopTotalKeys: TOTAL_KEYS, mobileTotalKeys: 42, locale: 'de' })
        : null;
    ...
    if (soundButton) soundButton.addEventListener('click', () => { state.soundEnabled = !state.soundEnabled; soundButton.classList.toggle('active', state.soundEnabled); soundButton.querySelector('.sound-label').textContent = state.soundEnabled ? 'Ton: An' : 'Ton: Aus'; });
...
function handleKeyDown(e) {
    ...
    function reset() { state.keyPressCount = {}; state.uniqueKeys.clear(); state.totalPresses = 0; state.sessionStart = null; state.latencyData = []; state.lastKeyTime = 0; if (state.sessionTimer) { clearInterval(state.sessionTimer); state.sessionTimer = null; } if ($('key-history')) $('key-history').value = ''; if ($('total-keys')) $('total-keys').textContent = '0'; if ($('session-time')) $('session-time').textContent = '00:00'; if ($('keys-tested')) $('keys-tested').textContent = `0/${TOTAL_KEYS}`; if ($('most-pressed')) $('most-pressed').textContent = 'Keine'; if ($('current-latency')) $('current-latency').textContent = '-- ms'; if ($('avg-latency')) $('avg-latency').textContent = '--'; if ($('min-latency')) $('min-latency').textContent = '--'; if ($('max-latency')) $('max-latency').textContent = '--'; $$('.key').forEach(k => { k.classList.remove('pressed', 'active-1', 'active-2', 'active-3', 'active-4', 'active-5'); const counter = k.querySelector('.key-counter'); if (counter) counter.remove(); }); $$('.indicator').forEach(i => i.classList.remove('active')); resetCatProgress(); }
...
if (!isset($GLOBALS['__kbt_adv_loaded'])) {
    ...
    echo "<script>window.KBT_LOCALE = 'de';</script>";
