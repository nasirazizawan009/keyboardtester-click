...
<div class="click-tester-section">
    <div class="click-tester">
        <div class="duration-select">
            <label for="duration">Testdauer auswählen: </label>
            <select id="duration">
        ...
        <div class="info-area">
            <div class="info-box">Dauer <span id="sel-dur">5s</span></div>
            <div class="info-box">Klicks <span id="cur-clicks">—</span></div>
            <div class="info-box">Zeit <span id="time-left">5s</span></div>
        </div>
        ...
        <div class="result-strip" id="result-strip" style="display:none">
            ...
            <div class="result-row">
                <span class="result-label">Klicks</span>
                <span class="result-val" id="res-clicks">—</span>
            </div>
            <p class="result-remark" id="res-remark"></p>
            <button class="new-test-btn" id="new-test-btn">Neuer Test</button>
        </div>
        ...
        <div class="test-history" id="test-history" style="display:none">
            <div class="history-header">
                <span>Verlauf</span>
                <button id="clear-history">Löschen</button>
            </div>
            <table class="history-table">
                <thead><tr><th>#</th><th>Dauer</th><th>Klicks</th><th>CPS</th></tr></thead>
...
(function () {
    ...
    function endTest() {
        ...
        clickArea.textContent = 'Test abgeschlossen';

        resCps.textContent    = cps;
        resClicks.textContent = `${clickCount} in ${dur}s`;
    ...
    function getRemark(cps) {
        if (cps > 14) return "Schmetterlings-Gott-Level. Bist du überhaupt menschlich?";
        if (cps > 10) return "Jitter-Klicken! Dein Unterarm ist anderer Meinung.";
        if (cps > 7)  return "Überdurchschnittlich — solides Gaming-Tempo!";
        if (cps > 4)  return "Durchschnittlicher Bereich. Noch Luft nach oben!";
        return "Schön langsam — wärme diese Finger auf!";
    }
