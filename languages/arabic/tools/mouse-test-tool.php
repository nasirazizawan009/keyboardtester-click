<?php
// Arabic Mouse Tester Tool - اختبار الماوس
?>
<div class="mouse-tester">
  <canvas id="mouse-canvas" width="400" height="250" role="button" aria-label="منطقة اختبار الماوس، انقر على أي زر أو مرر"></canvas>

  <div class="results">
    <div class="result-box" id="left-click-count" aria-live="polite">النقر الأيسر: 0</div>
    <div class="result-box" id="middle-click-count" aria-live="polite">النقر الأوسط: 0</div>
    <div class="result-box" id="right-click-count" aria-live="polite">النقر الأيمن: 0</div>
    <div class="result-box" id="scroll-count" aria-live="polite">التمرير: 0</div>
  </div>

  <div class="status">
    <div class="status-box" id="left-click-status">الأيسر: محرر (آخر: لا يوجد)</div>
    <div class="status-box" id="middle-click-status">الأوسط: محرر (آخر: لا يوجد)</div>
    <div class="status-box" id="right-click-status">الأيمن: محرر (آخر: لا يوجد)</div>
    <div class="status-box" id="scroll-status">التمرير: لا يوجد (آخر: لا يوجد)</div>
  </div>

  <button class="reset-button" id="reset-button" aria-label="إعادة تعيين جميع العدادات والحالات">إعادة التعيين</button>
</div>
...
<script>
  ...
  const buttonStates = {
    left: { status: 'محرر', lastPress: 'لا يوجد' },
    middle: { status: 'محرر', lastPress: 'لا يوجد' },
    right: { status: 'محرر', lastPress: 'لا يوجد' },
    scroll: { status: 'لا يوجد', lastPress: 'لا يوجد' }
  };
  ...
  function updateStatus(button, status, timestamp) {
    ...
    if (timestamp) buttonStates[button].lastPress = new Date().toLocaleTimeString('ar-SA');
    const buttonNames = { left: 'الأيسر', middle: 'الأوسط', right: 'الأيمن', scroll: 'التمرير' };
    const displayText = button === 'scroll' ?
      `التمرير: ${buttonStates[button].status} (آخر: ${buttonStates[button].lastPress})` :
      `${buttonNames[button]}: ${buttonStates[button].status} (آخر: ${buttonStates[button].lastPress})`;
    switch (button) {
      case 'left':
        leftClickStatus.textContent = displayText;
        leftClickStatus.classList.toggle('left-click-pressed', status === 'مضغوط');
        break;
      case 'middle':
        middleClickStatus.textContent = displayText;
        middleClickStatus.classList.toggle('middle-click-pressed', status === 'مضغوط');
        break;
      case 'right':
        rightClickStatus.textContent = displayText;
        rightClickStatus.classList.toggle('right-click-pressed', status === 'مضغوط');
        break;
      case 'scroll':
        scrollStatus.textContent = displayText;
        scrollStatus.classList.toggle('scroll-pressed', status !== 'لا يوجد');
        break;
    }
  }
  ...
  canvas.addEventListener('mousedown', (event) => {
    ...
    if (buttonType) {
      ...
      switch (buttonType) {
        case 'left':
          leftClickCount++;
          leftClickDisplay.textContent = `النقر الأيسر: ${leftClickCount}`;
          leftClickDisplay.classList.add('left-click-active');
          updateStatus('left', 'مضغوط', true);
          break;
        case 'middle':
          middleClickCount++;
          middleClickDisplay.textContent = `النقر الأوسط: ${middleClickCount}`;
          middleClickDisplay.classList.add('middle-click-active');
          updateStatus('middle', 'مضغوط', true);
          break;
        case 'right':
          rightClickCount++;
          rightClickDisplay.textContent = `النقر الأيمن: ${rightClickCount}`;
          rightClickDisplay.classList.add('right-click-active');
          updateStatus('right', 'مضغوط', true);
          break;
      }
    }
  });
  ...
  canvas.addEventListener('mouseup', (event) => {
    event.preventDefault();
    switch (event.button) {
      case 0: updateStatus('left', 'محرر'); break;
      case 1: updateStatus('middle', 'محرر'); break;
      case 2: updateStatus('right', 'محرر'); break;
    }
  });
  ...
  resetButton.addEventListener('click', () => {
    ...
    scrollOffset = 0;
    lastScrollDirection = null;
    buttonStates.left = { status: 'محرر', lastPress: 'لا يوجد' };
    buttonStates.middle = { status: 'محرر', lastPress: 'لا يوجد' };
    buttonStates.right = { status: 'محرر', lastPress: 'لا يوجد' };
    buttonStates.scroll = { status: 'لا يوجد', lastPress: 'لا يوجد' };
    leftClickDisplay.textContent = 'النقر الأيسر: 0';
    middleClickDisplay.textContent = 'النقر الأوسط: 0';
    rightClickDisplay.textContent = 'النقر الأيمن: 0';
    scrollDisplay.textContent = 'التمرير: 0';
    leftClickStatus.textContent = 'الأيسر: محرر (آخر: لا يوجد)';
    middleClickStatus.textContent = 'الأوسط: محرر (آخر: لا يوجد)';
    rightClickStatus.textContent = 'الأيمن: محرر (آخر: لا يوجد)';
    scrollStatus.textContent = 'التمرير: لا يوجد (آخر: لا يوجد)';
