<?php
// Arabic Mouse DPI / Sensitivity Tester tool - اختبار حساسية الماوس
?>

<div class="kbt-tool kbt-dpi-tool" id="dpi-tool" dir="rtl">
  <div class="kbt-tool-grid two">
    <div class="kbt-tool-card">
      <h3>تقدير DPI</h3>
      <p>أدخل المسافة الفعلية التي تحرك بها الماوس، ثم اسحب داخل منطقة التتبع.</p>
      <div class="kbt-tool-field">
        <label for="dpi-distance">المسافة المقطوعة</label>
      ...
      <div class="kbt-tool-actions">
        <button class="kbt-tool-button primary" id="dpi-start">بدء القياس</button>
        <button class="kbt-tool-button" id="dpi-reset">إعادة التعيين</button>
      </div>
    </div>
    <div class="kbt-tool-card">
      <h3>منطقة التتبع</h3>
      <div class="dpi-track" id="dpi-track" aria-label="منطقة تتبع DPI">
        اسحب هنا مع الضغط على زر الماوس.
      </div>
      <div class="kbt-tool-stats">
        <div class="kbt-tool-stat"><span>البكسلات المتحركة</span><strong id="dpi-pixels">0 بكسل</strong></div>
        <div class="kbt-tool-stat"><span>DPI المقدر</span><strong id="dpi-result">--</strong></div>
      </div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateDisplay() {
    pixelsEl.textContent = `${Math.round(totalPixels)} بكسل`;
