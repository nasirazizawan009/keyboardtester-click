<?php
// webcam-test-tool.php — embeddable Webcam Tester module (Spanish version)
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="wc-grid">
      ...
      <div class="wc-card">
        ...
        <div class="wc-video-container" id="wc-videoContainer">
          <div class="wc-placeholder" id="wc-placeholder">
            ...
            <p style="margin: 0; font-size: 16px; font-weight: 600;">Vista Previa de Webcam</p>
            <p style="margin: 8px 0 0; font-size: 13px;">Haz clic en "Iniciar Webcam" para comenzar la prueba</p>
          </div>
          ...
          <div class="wc-overlay" id="wc-overlay" style="display: none;">
            <div class="wc-badge">
              ...
              <span id="wc-statusText">En Vivo</span>
            </div>
            <div class="wc-badge" id="wc-resolutionBadge">Cargando...</div>
          </div>
        </div>

        <div class="wc-controls">
          <button id="wc-startBtn" class="wc-btn wc-btn-primary">Iniciar Webcam</button>
          <button id="wc-stopBtn" class="wc-btn wc-btn-danger" disabled>Detener Webcam</button>
          <button id="wc-snapshotBtn" class="wc-btn wc-btn-accent" disabled>Tomar Captura</button>
          <button id="wc-downloadBtn" class="wc-btn wc-btn-ghost" disabled>Descargar Todo</button>
        </div>

        <!-- Snapshots Section -->
        <div id="wc-snapshotsSection" style="display: none;">
          <h4 style="margin: 20px 0 0; font-size: 14px; color: var(--muted);">Capturas Guardadas (<span id="wc-snapshotCount">0</span>)</h4>
    ...
    <div>
      ...
      <div class="wc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Información de la Webcam</h3>
        <div class="wc-info-grid">
          <div class="wc-info-item">
            <span class="wc-info-label">Estado</span>
            <span class="wc-info-value" id="wc-infoStatus">No Iniciada</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Resolución</span>
            <span class="wc-info-value" id="wc-infoResolution">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Relación de Aspecto</span>
            <span class="wc-info-value" id="wc-infoAspect">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Tasa de Cuadros</span>
            <span class="wc-info-value" id="wc-infoFPS">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Dispositivo</span>
      ...
      <!-- Settings -->
      <div class="wc-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Configuración</h3>
        <div class="wc-settings">
          <div class="wc-setting-item">
            <label for="wc-cameraSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Seleccionar Cámara</label>
          ...
          <div class="wc-setting-item">
            <label for="wc-resolutionSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Resolución Preferida</label>
        ...
        <div style="margin-top: 20px;">
          <h4 style="margin: 0 0 12px; font-size: 14px; color: var(--muted);">Características</h4>
          <div class="wc-feature-list">
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Vista previa de video en tiempo real</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Soporte para múltiples cámaras</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Captura y descarga de fotos</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Detección de resolución</span>
            </div>
          </div>
        </div>
      </div>
...
(()=>{
  ...
  async function startWebcam() {
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        showInfo('¡Webcam iniciada correctamente!');
      };

    } catch (err) {
      console.error('Error accessing webcam:', err);
      showError('No se pudo acceder a la webcam. Por favor, verifica los permisos e intenta de nuevo.');
    }
  }
  ...
  function updateInfo(reset = false) {
    if (reset) {
      document.getElementById('wc-infoStatus').textContent = 'No Iniciada';
    ...
    document.getElementById('wc-infoStatus').textContent = 'Activa';
    ...
    document.getElementById('wc-infoDevice').textContent = videoTrack.label || 'Cámara Predeterminada';
  ...
  function downloadAll() {
    snapshots.forEach((snapshot, index) => {
      ...
      link.download = `captura-webcam-${index + 1}.png`;
      link.click();
    });
  }
  ...
  navigator.mediaDevices.getUserMedia({ video: true })
    ...
    .catch(() => {
      // Permission denied, cameras will show as "Cámara 1", "Cámara 2", etc.
    });
})();
</script>
