<?php
// headphone-test-tool.php – modulo de prueba de audifonos/altavoces integrable (sin shell <html>)
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="hs-grid">
    ...
    <div>
      <div class="hs-card">
        <div id="hs-infoAlert" class="hs-alert hs-alert-info">
          🎧 <strong>Consejo:</strong> Usa audifonos para la mejor experiencia de prueba estereo. Ajusta el volumen de tu sistema a un nivel comodo antes de comenzar.
        </div>

        <div class="hs-visualizer" id="hs-visualizer">
          <div class="hs-placeholder" id="hs-placeholder">
            <div class="hs-placeholder-icon">🎵</div>
            <p style="margin: 0; font-size: 16px; font-weight: 600;">Visualizador de Audio</p>
            <p style="margin: 8px 0 0; font-size: 13px;">Selecciona una prueba abajo para comenzar</p>
          </div>
          ...
          <div class="hs-overlay" id="hs-overlay" style="display: none;">
            <div class="hs-badge">
              ...
              <span id="hs-statusText">Reproduciendo</span>
            </div>
        ...
        <div class="hs-controls">
          <button id="hs-stopBtn" class="hs-btn hs-btn-danger" disabled>Detener Audio</button>
        </div>
    ...
    <div>
      ...
      <div class="hs-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Prueba Rapida de Canales</h3>
        <div class="hs-channel-grid">
          <button class="hs-channel-btn" id="hs-leftBtn">
            <div style="font-size: 24px; margin-bottom: 8px;">⬅️</div>
            <div>Canal Izquierdo</div>
          </button>
          <button class="hs-channel-btn" id="hs-rightBtn">
            <div style="font-size: 24px; margin-bottom: 8px;">➡️</div>
            <div>Canal Derecho</div>
          </button>
        </div>
        <button class="hs-channel-btn" id="hs-stereoBtn" style="margin-top: 12px; width: 100%;">
          <div style="font-size: 24px; margin-bottom: 8px;">🔊</div>
          <div>Ambos Canales (Estereo)</div>
        </button>
      </div>

      <!-- Audio Tests -->
      <div class="hs-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Pruebas de Audio</h3>
        <div class="hs-test-grid">
          <div class="hs-test-item" data-test="sweep">
            <div class="hs-test-title">🎼 Barrido de Frecuencia</div>
            <div class="hs-test-desc">Barrido de 20Hz a 20kHz para probar el rango completo de audio</div>
          </div>
          <div class="hs-test-item" data-test="bass">
            <div class="hs-test-title">🔉 Prueba de Graves (100Hz)</div>
            <div class="hs-test-desc">Prueba la respuesta de baja frecuencia y el subwoofer</div>
          </div>
          <div class="hs-test-item" data-test="mid">
            <div class="hs-test-title">🔊 Rango Medio (1kHz)</div>
            <div class="hs-test-desc">Prueba la claridad de voces e instrumentos</div>
          </div>
          <div class="hs-test-item" data-test="treble">
            <div class="hs-test-title">🔔 Prueba de Agudos (10kHz)</div>
            <div class="hs-test-desc">Prueba la respuesta y claridad de alta frecuencia</div>
          </div>
          <div class="hs-test-item" data-test="balance">
            <div class="hs-test-title">⚖️ Prueba de Balance</div>
            <div class="hs-test-desc">Alterna entre los canales izquierdo y derecho</div>
          </div>
          <div class="hs-test-item" data-test="noise">
            <div class="hs-test-title">📻 Ruido Blanco</div>
            <div class="hs-test-desc">Ruido de espectro completo para prueba de altavoces</div>
          </div>
        </div>
      </div>
      ...
      <div class="hs-card" style="margin-top: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Informacion de Audio</h3>
        <div class="hs-info-grid">
          <div class="hs-info-item">
            <span class="hs-info-label">Estado</span>
            <span class="hs-info-value" id="hs-infoStatus">Listo</span>
          </div>
          <div class="hs-info-item">
            <span class="hs-info-label">Prueba Actual</span>
            <span class="hs-info-value" id="hs-infoTest">Ninguna</span>
          </div>
          <div class="hs-info-item">
            <span class="hs-info-label">Frecuencia</span>
...
(()=>{
  ...
  // Play tone
  function playTone(frequency, panValue = 0, channelName = 'Estereo') {
    stopAudio();
    initAudioContext();
    ...
    updateInfo(false, `Tono de ${frequency}Hz`, frequency, channelName);
  }
  ...
  function playNoise() {
    ...
    updateInfo(false, 'Ruido Blanco', 'Espectro Completo', 'Estereo');
  }
  ...
  function playSweep() {
    ...
    updateInfo(false, 'Barrido de Frecuencia', '20Hz - 20kHz', 'Estereo');
  }
  ...
  function playBalance() {
    ...
    const balanceInterval = setInterval(() => {
      ...
      const channelName = panValue === -1 ? 'Izquierdo' : 'Derecho';
    ...
    updateInfo(false, 'Prueba de Balance', '440Hz', 'Alternando');
  }
  ...
  function updateInfo(reset = false, testName = '', frequency = '', channel = '') {
    if (reset) {
      infoStatus.textContent = 'Listo';
      infoStatus.style.color = 'var(--muted)';
      infoTest.textContent = 'Ninguna';
    ...
    infoStatus.textContent = 'Reproduciendo';
    ...
    item.addEventListener('click', function() {
      ...
      switch(testType) {
        case 'sweep':
          ...
          break;
        case 'bass':
          playTone(100, 0, 'Estereo');
          break;
        case 'mid':
          playTone(1000, 0, 'Estereo');
          break;
        case 'treble':
          playTone(10000, 0, 'Estereo');
          break;
        case 'balance':
          playBalance();
          break;
