<?php
// screen-test-tool.php — embeddable Screen Test module (Spanish version)
?>
<style>
  :root {
    --bg: #f6f8fc;
    ...
    <div>
      <div class="st-card">
        <h2 style="margin: 0 0 16px 0; font-size: 20px; color: var(--text); display: flex; align-items: center; gap: 10px;">
          ...
          Vista Previa de Prueba de Pantalla
        </h2>
        ...
        <div class="st-preview-container" id="st-preview-box" onclick="startScreenTest()">
          <div class="st-preview" id="st-preview-display" style="background: #000;">
            Haz clic para iniciar la prueba de pantalla completa
          </div>
          <div class="st-overlay">
            <div class="st-badge">
              <span id="st-current-color">Listo</span>
            </div>
            <div class="st-badge">
              Haz clic en cualquier lugar para probar
            </div>
          </div>
        </div>

        <div class="st-colors-grid">
          <button class="st-color-btn" style="background: #000;" onclick="setPreviewColor('#000000', 'Negro')" title="Negro - Mejor para encontrar píxeles muertos">
            Negro
          </button>
          <button class="st-color-btn" style="background: #fff; color: #000; text-shadow: none;" onclick="setPreviewColor('#FFFFFF', 'Blanco')" title="Blanco - Mejor para encontrar píxeles atascados">
            Blanco
          </button>
          <button class="st-color-btn" style="background: #ff0000;" onclick="setPreviewColor('#FF0000', 'Rojo')" title="Rojo - Verificar subpíxeles rojos">
            Rojo
          </button>
          <button class="st-color-btn" style="background: #00ff00;" onclick="setPreviewColor('#00FF00', 'Verde')" title="Verde - Verificar subpíxeles verdes">
            Verde
          </button>
          <button class="st-color-btn" style="background: #0000ff;" onclick="setPreviewColor('#0000FF', 'Azul')" title="Azul - Verificar subpíxeles azules">
            Azul
          </button>
          <button class="st-color-btn" style="background: #ffff00; color: #000; text-shadow: none;" onclick="setPreviewColor('#FFFF00', 'Amarillo')" title="Amarillo - Subpíxeles R+V">
            Amarillo
          </button>
          <button class="st-color-btn" style="background: #ff00ff;" onclick="setPreviewColor('#FF00FF', 'Magenta')" title="Magenta - Subpíxeles R+A">
            Magenta
          </button>
          <button class="st-color-btn" style="background: #00ffff; color: #000; text-shadow: none;" onclick="setPreviewColor('#00FFFF', 'Cian')" title="Cian - Subpíxeles V+A">
            Cian
          </button>
        </div>

        <div class="st-alert st-alert-info">
          <strong>💡 Consejo:</strong> ¡Limpia tu pantalla primero! Usa las teclas de flecha (← →) o haz clic para cambiar entre colores en modo pantalla completa. Presiona ESC para salir.
        </div>
      </div>
    </div>
    ...
    <div>
      <div class="st-card">
        ...
        <div class="st-stats-grid">
          ...
          <div class="st-stat-card">
            <div class="label">Colores Probados</div>
        ...
        <div class="st-section">
          ...
          <ul class="st-tips-list">
            <li><strong>Píxeles Muertos:</strong> Puntos negros permanentes (en todos los colores)</li>
            <li><strong>Píxeles Atascados:</strong> Puntos fijos en un solo color (rojo/verde/azul)</li>
            <li><strong>Píxeles Calientes:</strong> Píxeles siempre blancos</li>
            <li><strong>Uniformidad de Color:</strong> Color uniforme en toda la pantalla</li>
          </ul>
        </div>
        ...
        <div class="st-section">
          ...
          <ul class="st-tips-list">
            <li>Limpia tu pantalla con un paño de microfibra</li>
            <li>Prueba en una habitación oscura o con poca luz</li>
            <li>Mira desde tu distancia normal</li>
            <li>Prueba los 8 colores a fondo</li>
            <li>Revisa cuidadosamente los bordes y esquinas</li>
            <li>Tómate tu tiempo - no te apresures</li>
          </ul>
        </div>

        <div class="st-alert st-alert-success">
          <strong>🔒 Privacidad Primero:</strong> Esta prueba se ejecuta completamente en tu navegador. No se recopilan ni envían datos a ningún servidor.
        </div>
      </div>
    </div>
  </div>
...
<div class="st-fullscreen-test" id="st-fullscreen-test">
  <div class="st-fs-instructions" id="st-fs-instructions">
    <div style="font-weight: 600; margin-bottom: 8px;">Prueba de Pantalla Activa</div>
    <div style="font-size: 12px;">Usa las teclas de flecha (← →) o haz clic para cambiar colores • Presiona ESC para salir</div>
  </div>
  <div class="st-fs-controls">
    <button class="st-fs-btn" onclick="previousColor()">← Anterior</button>
    <button class="st-fs-btn" onclick="nextColor()">Siguiente →</button>
    <button class="st-fs-btn" onclick="exitFullScreen()">Salir (ESC)</button>
  </div>
</div>

<script>
  const colors = [
    { hex: '#000000', name: 'Negro', desc: 'Busca píxeles muertos (puntos negros)' },
    { hex: '#FFFFFF', name: 'Blanco', desc: 'Busca píxeles atascados (puntos de color)' },
    { hex: '#FF0000', name: 'Rojo', desc: 'Verifica subpíxeles rojos' },
    { hex: '#00FF00', name: 'Verde', desc: 'Verifica subpíxeles verdes' },
    { hex: '#0000FF', name: 'Azul', desc: 'Verifica subpíxeles azules' },
    { hex: '#FFFF00', name: 'Amarillo', desc: 'Subpíxeles Rojo + Verde' },
    { hex: '#FF00FF', name: 'Magenta', desc: 'Subpíxeles Rojo + Azul' },
    { hex: '#00FFFF', name: 'Cian', desc: 'Subpíxeles Verde + Azul' }
  ];
