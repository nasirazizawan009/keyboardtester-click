<?php
// Herramienta de prueba de DPI v2.0 — Español
$dpi_strings = [
  'tab_test'   => 'Prueba DPI',
  'tab_edpi'   => 'Calculadora eDPI',
  'tab_conv'   => 'Convertidor',
  'setup'      => '1 — Configuración',
  'lbl_dist'   => 'Distancia física que moverás el ratón',
  'unit_cm'    => 'cm',
  'unit_in'    => 'pulgadas',
  'lbl_exp'    => 'DPI esperado',
  'exp_hint'   => '(opcional — activa desviación y precisión)',
  'lbl_axis'   => 'Restringir eje de medición',
  'axis_both'  => 'Cualquier dirección (recomendado)',
  'axis_x'     => 'Solo horizontal (eje X)',
  'axis_y'     => 'Solo vertical (eje Y)',
  'btn_start'  => '▶ Iniciar Medición',
  'btn_reset'  => 'Reiniciar Todo',
  'tip'        => '<strong>Consejo:</strong> Usa una regla sobre el mousepad. Marca inicio y fin, luego arrastra por el área de seguimiento manteniendo el botón presionado.',
  'track_lbl'  => 'Área de seguimiento de DPI',
  'track_title'=> '2 — Seguimiento y Resultados',
  'track_idle' => 'Presiona Iniciar, luego arrastra aquí',
  'track_ready'=> 'Mantén botón + arrastra la distancia medida',
  'stat_px'    => 'Píxeles recorridos',
  'stat_dpi'   => 'DPI Estimado',
  'stat_dev'   => 'Desviación',
  'stat_acc'   => 'Precisión',
  'runs_lbl'   => 'Historial de intentos (arrastra para agregar)',
  'run_n'      => 'Intento',
  'run_avg'    => 'Prom:',
  'run_more'   => ' de %d — arrastra de nuevo ↔',
  'run_done'   => 'Todos los intentos completos — ver promedio arriba',
  'privacy'    => '🔒 Esta prueba se ejecuta completamente en tu navegador. No se recopilan datos.',
  'edpi_setup' => 'Tu Configuración',
  'lbl_dpi'    => 'DPI del ratón',
  'lbl_sens'   => 'Sensibilidad en juego',
  'lbl_game'   => 'Juego (para comparación)',
  'game_any'   => '— Cualquiera / Personalizado —',
  'edpi_lbl'   => 'eDPI — DPI Efectivo',
  'edpi_formula'=> '<strong>Fórmula:</strong> eDPI = DPI del ratón × Sensibilidad en juego',
  'pro_title'  => 'Comparación de Rangos Pro',
  'pro_intro'  => 'Tu eDPI está resaltado en la tabla.',
  'pro_h1'     => 'Rango eDPI', 'pro_h2' => 'Nivel', 'pro_h3' => 'Común en',
  'conv_curr'  => 'Configuración Actual',
  'lbl_old_dpi'=> 'DPI actual',
  'lbl_old_s'  => 'Sensibilidad actual en juego',
  'conv_target'=> 'Configuración Objetivo',
  'lbl_new_dpi'=> 'Nuevo DPI (tras cambio de ratón)',
  'conv_sub'   => 'Nueva sensibilidad — mismo tacto muscular, diferente DPI',
  'conv_how'   => 'Cómo Funciona',
  'conv_exp1'  => 'El convertidor mantiene tu <strong>eDPI constante</strong>, para que el mismo movimiento físico siempre mueva tu vista en juego la misma distancia.',
  'conv_formula'=> 'nueva_sens = (DPI_viejo × sens_viejo) / DPI_nuevo',
  'conv_exp2'  => '<strong>Ejemplo:</strong> 400 DPI a 2.0 sens → 800 DPI a 1.0 sens. Ambos tienen eDPI 800.',
  'log_title'  => 'Registro de Sesión',
  'log_dl'     => '&#11015; Descargar Registro (.txt)',
  'log_empty'  => 'Sin intentos aún. Completa una prueba DPI para comenzar.',
];
?>
...
<div class="kbt-tool dpi-v2" id="dpi-tool">
  ...
  <div class="dpi-panel active" id="dpi-p-test" role="tabpanel">
    <div class="dpi-mode-row">
      <div class="dpi-mode-toggle">
        <button class="dpi-mode-btn active" data-mode="basic">Básico</button>
        <button class="dpi-mode-btn"        data-mode="advanced">Avanzado</button>
      </div>
    </div>
    <div class="dpi-grid">
      <div class="dpi-card">
        ...
        <div class="dpi-field">
          <label for="dpi-distance"><?php echo $dpi_strings['lbl_dist']; ?></label>
          <div class="dpi-inline">
            <input class="dpi-input" id="dpi-distance" type="number" min="1" max="100" step="0.5" value="10">
            <select class="dpi-select" id="dpi-unit">
              <option value="cm" selected><?php echo $dpi_strings['unit_cm']; ?></option>
              <option value="in"><?php echo $dpi_strings['unit_in']; ?></option>
            </select>
          </div>
        </div>
        <div class="dpi-field">
          <label for="dpi-expected"><?php echo $dpi_strings['lbl_exp']; ?> <small><?php echo $dpi_strings['exp_hint']; ?></small></label>
          <input class="dpi-input full" id="dpi-expected" type="number" min="100" max="32000" step="100" placeholder="ej. 800">
        </div>
        <div class="dpi-adv-section" id="dpi-adv">
          <div class="dpi-field">
            <label for="dpi-axis"><?php echo $dpi_strings['lbl_axis']; ?></label>
            <select class="dpi-select" id="dpi-axis" style="width:100%">
              <option value="both" selected><?php echo $dpi_strings['axis_both']; ?></option>
              <option value="x"><?php echo $dpi_strings['axis_x']; ?></option>
              <option value="y"><?php echo $dpi_strings['axis_y']; ?></option>
            </select>
          </div>
        </div>
        <div class="dpi-actions">
          <button class="dpi-btn primary" id="dpi-start"><?php echo $dpi_strings['btn_start']; ?></button>
          <button class="dpi-btn ghost"   id="dpi-reset"><?php echo $dpi_strings['btn_reset']; ?></button>
        </div>
      ...
      <div class="dpi-card">
        ...
        <div class="dpi-track" id="dpi-track" aria-label="<?php echo $dpi_strings['track_lbl']; ?>">
          <span id="dpi-track-msg"><?php echo $dpi_strings['track_idle']; ?></span>
        ...
        <div class="dpi-runs" id="dpi-runs" style="display:none" aria-live="polite">
          <p class="dpi-runs-lbl"><?php echo $dpi_strings['runs_lbl']; ?></p>
  ...
  <div class="dpi-panel" id="dpi-p-edpi" role="tabpanel">
    <div class="dpi-grid">
      <div class="dpi-card">
        ...
        <div class="dpi-field">
          <label for="edpi-game"><?php echo $dpi_strings['lbl_game']; ?></label>
          <select class="dpi-select" id="edpi-game" style="width:100%">
            <option value=""><?php echo $dpi_strings['game_any']; ?></option>
            <option value="cs2">CS2 / CS:GO</option><option value="valorant">Valorant</option>
            <option value="apex">Apex Legends</option><option value="fortnite">Fortnite</option><option value="overwatch">Overwatch 2</option>
          </select>
        </div>
      ...
      <div class="dpi-card">
        ...
        <tbody id="edpi-tbody">
          <tr data-min="0"    data-max="400">   <td>200–400</td>  <td>Ultra Bajo</td>  <td>Pro FPS (S1mple, sh1ro)</td></tr>
          <tr data-min="400"  data-max="800">   <td>400–800</td>  <td>Bajo</td>        <td>FPS competitivo / pros Valorant</td></tr>
          <tr data-min="800"  data-max="1600">  <td>800–1600</td> <td>Medio</td>       <td>Gaming general, FPS casual</td></tr>
          <tr data-min="1600" data-max="3200">  <td>1600–3200</td><td>Alto</td>        <td>MOBA / RTS</td></tr>
          <tr data-min="3200" data-max="999999"><td>3200+</td>    <td>Muy Alto</td>    <td>Oficina, diseño, casual</td></tr>
        </tbody></table>
      </div>
    </div>
    <p class="dpi-privacy"><?php echo $dpi_strings['privacy']; ?></p>
    ...
    <div class="dpi-grid">
      <div class="dpi-card">
        ...
        <div class="dpi-conv-result">
          <p class="dpi-conv-formula" id="conv-formula">nueva_sens = (400 &times; 2.0) / 800</p>
...
function accuracy(p){if(p<=2)return{label:'Excelente ✓',cls:'s-good'};if(p<=5)return{label:'Bueno ✓',cls:'s-good'};if(p<=10)return{label:'Regular',cls:'s-warn'};return{label:'Deficiente',cls:'s-bad'};}
...
function updConv(){var od=parseFloat(cOd.value)||0,os=parseFloat(cOs.value)||0,nd=parseFloat(cNd.value)||0;if(od&&os&&nd){cFm.innerHTML='nueva_sens = ('+od+' &times; '+os+') / '+nd;cVl.textContent=((od*os)/nd).toFixed(4);}else{cVl.textContent='--';}}
