<?php
/**
 * French Keyboard Tester - Full Layout Tool (AZERTY)
 */
...
<section class="keyboard-tester" id="keyboard-tester">
    <div class="controls-wrapper">
        <div class="textarea-wrapper">
            <div class="textarea-label">Historique des Touches</div>
            <textarea class="text-box" id="key-history" rows="4" placeholder="Appuyez sur une touche pour commencer le test..."></textarea>
        </div>
    ...
    <div class="feature-controls" id="feature-controls" style="display: none;">
        ...
        <button class="feature-button" id="sound-button">
            ...
            <span class="sound-label">Son: Desactive</span>
        </button>
    ...
    <div class="stats-panel" id="stats-panel" style="display: none;">
        <div class="stats-header">
            <h3><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 20V10M12 20V4M6 20v-6"/></svg> Statistiques de Session</h3>
            <button class="close-btn" id="close-stats">&times;</button>
        </div>
        <div class="stats-content">
            <div class="stat-item"><span class="stat-label">Total de Frappes</span><span class="stat-value" id="total-keys">0</span></div>
            <div class="stat-item"><span class="stat-label">Duree de Session</span><span class="stat-value" id="session-time">00:00</span></div>
            <div class="stat-item"><span class="stat-label">Touches Testees</span><span class="stat-value" id="keys-tested">0/104</span></div>
            <div class="stat-item"><span class="stat-label">Plus Appuyee</span><span class="stat-value" id="most-pressed">Aucune</span></div>
        </div>
    </div>

    <div class="stats-panel" id="latency-panel" style="display: none;">
        <div class="stats-header">
            <h3><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> Test de Latence</h3>
        ...
        <div class="stats-content">
            <div class="stat-item latency-display">
                ...
                <div class="latency-sublabel">Latence Actuelle</div>
            </div>
            <div class="latency-grid">
                <div class="stat-item compact"><span class="stat-value mono" id="avg-latency">--</span><span class="stat-label">Moyenne</span></div>
                <div class="stat-item compact"><span class="stat-value mono" id="min-latency">--</span><span class="stat-label">Meilleure</span></div>
                <div class="stat-item compact"><span class="stat-value mono" id="max-latency">--</span><span class="stat-label">Pire</span></div>
            </div>
        </div>
    </div>
    ...
    <div class="keyboard-container">
        ...
        <div class="keyboard-layout">
            <div class="keyboard-section main-keyboard">
                ...
                <!-- AZERTY Top Letter Row -->
                <div class="key-row">
                    <button class="key key-1-5x" data-key="Tab"><span>Tab</span></button>
                    <button class="key" data-key="KeyQ"><span>A</span></button>
                    <button class="key" data-key="KeyW"><span>Z</span></button>
                    ...
                    <button class="key key-1-5x" data-key="Backslash"><span class="dual">µ<br>*</span></button>
                </div>
                <!-- AZERTY Home Row -->
                <div class="key-row">
                    <button class="key key-1-75x" data-key="CapsLock"><span>Verr Maj</span></button>
                    <button class="key" data-key="KeyA"><span>Q</span></button>
                ...
                <div class="key-row">
                    <button class="key key-2-25x" data-key="ShiftLeft"><span>Maj</span></button>
                    <button class="key" data-key="KeyZ"><span>W</span></button>
                ...
                <div class="key-row">
                    ...
                    <button class="key key-space" data-key="Space"><span>Espace</span></button>
            ...
            <div class="keyboard-section nav-section">
                <div class="nav-top">
                    <div class="key-row">
                        ...
                        <button class="key" data-key="ScrollLock"><span>Defil</span></button>
            ...
            <div class="keyboard-section numpad-section">
                <div class="indicator-panel">
                    <div class="indicator" id="caps-indicator"><div class="led" id="caps-led"></div><span>Maj</span></div>
                    <div class="indicator" id="num-indicator"><div class="led" id="num-led"></div><span>Num</span></div>
                    <div class="indicator" id="scroll-indicator"><div class="led" id="scroll-led"></div><span>Defil</span></div>
                </div>
                <div class="numpad-grid">
                    <div class="key-row">
                        <button class="key" data-key="NumLock"><span>Verr Num</span></button>
                    ...
                    <div class="key-row">
                        ...
                        <button class="key key-tall key-enter-tall" data-key="NumpadEnter"><span>Entree</span></button>
                    </div>
                    <div class="key-row">
...
(function() {
    ...
    const catProgress = (catProgressRoot && window.KeyboardCatProgress)
        ? new window.KeyboardCatProgress(catProgressRoot, { desktopTotalKeys: TOTAL_KEYS, mobileTotalKeys: 42, locale: 'fr' })
        : null;
    ...
    if (soundButton) soundButton.addEventListener('click', () => { state.soundEnabled = !state.soundEnabled; soundButton.classList.toggle('active', state.soundEnabled); soundButton.querySelector('.sound-label').textContent = state.soundEnabled ? 'Son: Active' : 'Son: Desactive'; });
...
function handleKeyDown(e) {
    ...
    function reset() { state.keyPressCount = {}; state.uniqueKeys.clear(); state.totalPresses = 0; state.sessionStart = null; state.latencyData = []; state.lastKeyTime = 0; if (state.sessionTimer) { clearInterval(state.sessionTimer); state.sessionTimer = null; } if ($('key-history')) $('key-history').value = ''; if ($('total-keys')) $('total-keys').textContent = '0'; if ($('session-time')) $('session-time').textContent = '00:00'; if ($('keys-tested')) $('keys-tested').textContent = `0/${TOTAL_KEYS}`; if ($('most-pressed')) $('most-pressed').textContent = 'Aucune'; if ($('current-latency')) $('current-latency').textContent = '-- ms'; if ($('avg-latency')) $('avg-latency').textContent = '--'; if ($('min-latency')) $('min-latency').textContent = '--'; if ($('max-latency')) $('max-latency').textContent = '--'; $$('.key').forEach(k => { k.classList.remove('pressed', 'active-1', 'active-2', 'active-3', 'active-4', 'active-5'); const counter = k.querySelector('.key-counter'); if (counter) counter.remove(); }); $$('.indicator').forEach(i => i.classList.remove('active')); resetCatProgress(); }
...
if (!isset($GLOBALS['__kbt_adv_loaded'])) {
    ...
    echo "<script>window.KBT_LOCALE = 'fr';</script>";
