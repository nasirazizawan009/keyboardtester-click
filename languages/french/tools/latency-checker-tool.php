<?php
// Latency Checker tool (standalone) - French Version
?>
...
<div class="kbt-tool kbt-latency-tool" id="latency-tool">
  <div class="kbt-tool-grid two">
    <div class="kbt-tool-card">
      ...
      <p>Appuyez sur n'importe quelle touche pendant que le test est actif. Nous estimons le delai de traitement pour chaque frappe.</p>
      <div class="kbt-tool-actions">
        <button class="kbt-tool-button primary" id="latency-start">Demarrer le test</button>
        <button class="kbt-tool-button" id="latency-stop" disabled>Arreter</button>
        <button class="kbt-tool-button ghost" id="latency-clear">Effacer</button>
      </div>
      <div class="kbt-tool-status" id="latency-status">Etat: Pret</div>
    </div>
    <div class="kbt-tool-card">
      <h3>Resultats</h3>
      <div class="kbt-tool-stats">
        <div class="kbt-tool-stat"><span>Actuel</span><strong id="latency-current">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Moyenne</span><strong id="latency-avg">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Meilleur</span><strong id="latency-min">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Pire</span><strong id="latency-max">-- ms</strong></div>
        <div class="kbt-tool-stat"><span>Echantillons</span><strong id="latency-count">0</strong></div>
      </div>
      <div class="kbt-tool-log" id="latency-log">Appuyez sur Demarrer puis appuyez sur des touches pour capturer des echantillons.</div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  startBtn.addEventListener('click', function () {
    active = true;
    statusEl.textContent = 'Etat: Test en cours (appuyez sur des touches)';
  ...
  clearBtn.addEventListener('click', function () {
    ...
    logEl.textContent = 'Appuyez sur Demarrer puis appuyez sur des touches pour capturer des echantillons.';
  });
