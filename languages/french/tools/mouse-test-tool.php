<?php
// French Mouse Tester Tool - Testeur de Souris
?>
<div class="mouse-tester">
  <canvas id="mouse-canvas" width="400" height="250" role="button" aria-label="Zone de test de la souris, cliquez sur n'importe quel bouton ou faites defiler"></canvas>

  <div class="results">
    <div class="result-box" id="left-click-count" aria-live="polite">Clic Gauche: 0</div>
    <div class="result-box" id="middle-click-count" aria-live="polite">Clic Milieu: 0</div>
    <div class="result-box" id="right-click-count" aria-live="polite">Clic Droit: 0</div>
    <div class="result-box" id="scroll-count" aria-live="polite">Defilement: 0</div>
  </div>

  <div class="status">
    <div class="status-box" id="left-click-status">Gauche: Relache (Dernier: Aucun)</div>
    <div class="status-box" id="middle-click-status">Milieu: Relache (Dernier: Aucun)</div>
    <div class="status-box" id="right-click-status">Droit: Relache (Dernier: Aucun)</div>
    <div class="status-box" id="scroll-status">Defilement: Aucun (Dernier: Aucun)</div>
  </div>

  <button class="reset-button" id="reset-button" aria-label="Reinitialiser tous les compteurs et etats">Reinitialiser</button>
</div>
...
<script>
  ...
  const buttonStates = {
    left: { status: 'Relache', lastPress: 'Aucun' },
    middle: { status: 'Relache', lastPress: 'Aucun' },
    right: { status: 'Relache', lastPress: 'Aucun' },
    scroll: { status: 'Aucun', lastPress: 'Aucun' }
  };
  ...
  function updateStatus(button, status, timestamp) {
    ...
    if (timestamp) buttonStates[button].lastPress = new Date().toLocaleTimeString('fr-FR');
    const buttonNames = { left: 'Gauche', middle: 'Milieu', right: 'Droit', scroll: 'Defilement' };
    const displayText = button === 'scroll' ?
      `Defilement: ${buttonStates[button].status} (Dernier: ${buttonStates[button].lastPress})` :
      `${buttonNames[button]}: ${buttonStates[button].status} (Dernier: ${buttonStates[button].lastPress})`;
    switch (button) {
      case 'left':
        leftClickStatus.textContent = displayText;
        leftClickStatus.classList.toggle('left-click-pressed', status === 'Presse');
        break;
      case 'middle':
        middleClickStatus.textContent = displayText;
        middleClickStatus.classList.toggle('middle-click-pressed', status === 'Presse');
        break;
      case 'right':
        rightClickStatus.textContent = displayText;
        rightClickStatus.classList.toggle('right-click-pressed', status === 'Presse');
        break;
      case 'scroll':
        scrollStatus.textContent = displayText;
        scrollStatus.classList.toggle('scroll-pressed', status !== 'Aucun');
        break;
    }
  }
  ...
  canvas.addEventListener('mousedown', (event) => {
    ...
    if (buttonType) {
      ...
      switch (buttonType) {
        case 'left':
          leftClickCount++;
          leftClickDisplay.textContent = `Clic Gauche: ${leftClickCount}`;
          leftClickDisplay.classList.add('left-click-active');
          updateStatus('left', 'Presse', true);
          break;
        case 'middle':
          middleClickCount++;
          middleClickDisplay.textContent = `Clic Milieu: ${middleClickCount}`;
          middleClickDisplay.classList.add('middle-click-active');
          updateStatus('middle', 'Presse', true);
          break;
        case 'right':
          rightClickCount++;
          rightClickDisplay.textContent = `Clic Droit: ${rightClickCount}`;
          rightClickDisplay.classList.add('right-click-active');
          updateStatus('right', 'Presse', true);
          break;
      }
    }
  });
  ...
  canvas.addEventListener('mouseup', (event) => {
    event.preventDefault();
    switch (event.button) {
      case 0: updateStatus('left', 'Relache'); break;
      case 1: updateStatus('middle', 'Relache'); break;
      case 2: updateStatus('right', 'Relache'); break;
    }
  });
  ...
  resetButton.addEventListener('click', () => {
    ...
    scrollOffset = 0;
    lastScrollDirection = null;
    buttonStates.left = { status: 'Relache', lastPress: 'Aucun' };
    buttonStates.middle = { status: 'Relache', lastPress: 'Aucun' };
    buttonStates.right = { status: 'Relache', lastPress: 'Aucun' };
    buttonStates.scroll = { status: 'Aucun', lastPress: 'Aucun' };
    leftClickDisplay.textContent = 'Clic Gauche: 0';
    middleClickDisplay.textContent = 'Clic Milieu: 0';
    rightClickDisplay.textContent = 'Clic Droit: 0';
    scrollDisplay.textContent = 'Defilement: 0';
    leftClickStatus.textContent = 'Gauche: Relache (Dernier: Aucun)';
    middleClickStatus.textContent = 'Milieu: Relache (Dernier: Aucun)';
    rightClickStatus.textContent = 'Droit: Relache (Dernier: Aucun)';
    scrollStatus.textContent = 'Defilement: Aucun (Dernier: Aucun)';
