<?php
// webcam-test-tool.php - embeddable Webcam Tester module (French version)
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="wc-grid">
      ...
      <div class="wc-card">
        ...
        <div class="wc-video-container" id="wc-videoContainer">
          <div class="wc-placeholder" id="wc-placeholder">
            ...
            <p style="margin: 0; font-size: 16px; font-weight: 600;">Apercu de la Webcam</p>
            <p style="margin: 8px 0 0; font-size: 13px;">Cliquez sur "Demarrer la Webcam" pour commencer le test</p>
          </div>
          ...
          <div class="wc-overlay" id="wc-overlay" style="display: none;">
            <div class="wc-badge">
              ...
              <span id="wc-statusText">En Direct</span>
            </div>
            <div class="wc-badge" id="wc-resolutionBadge">Chargement...</div>
          </div>
        </div>

        <div class="wc-controls">
          <button id="wc-startBtn" class="wc-btn wc-btn-primary">Demarrer la Webcam</button>
          <button id="wc-stopBtn" class="wc-btn wc-btn-danger" disabled>Arreter la Webcam</button>
          <button id="wc-snapshotBtn" class="wc-btn wc-btn-accent" disabled>Prendre une Capture</button>
          <button id="wc-downloadBtn" class="wc-btn wc-btn-ghost" disabled>Tout Telecharger</button>
        </div>

        <!-- Snapshots Section -->
        <div id="wc-snapshotsSection" style="display: none;">
          <h4 style="margin: 20px 0 0; font-size: 14px; color: var(--muted);">Captures Enregistrees (<span id="wc-snapshotCount">0</span>)</h4>
    ...
    <div>
      ...
      <div class="wc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Informations de la Webcam</h3>
        <div class="wc-info-grid">
          <div class="wc-info-item">
            <span class="wc-info-label">Etat</span>
            <span class="wc-info-value" id="wc-infoStatus">Non Demarree</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Resolution</span>
            <span class="wc-info-value" id="wc-infoResolution">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Ratio d'Aspect</span>
            <span class="wc-info-value" id="wc-infoAspect">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Images par Seconde</span>
            <span class="wc-info-value" id="wc-infoFPS">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Appareil</span>
      ...
      <!-- Settings -->
      <div class="wc-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Parametres</h3>
        <div class="wc-settings">
          <div class="wc-setting-item">
            <label for="wc-cameraSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Selectionner la Camera</label>
            <select id="wc-cameraSelect" class="wc-select">
              <option value="">Chargement des cameras...</option>
            </select>
          </div>

          <div class="wc-setting-item">
            <label for="wc-resolutionSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Resolution Preferee</label>
            <select id="wc-resolutionSelect" class="wc-select">
              <option value="default">Automatique (Par defaut)</option>
        ...
        <div style="margin-top: 20px;">
          <h4 style="margin: 0 0 12px; font-size: 14px; color: var(--muted);">Fonctionnalites</h4>
          <div class="wc-feature-list">
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Apercu video en temps reel</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Support multi-cameras</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Capture et telechargement de photos</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Detection de resolution</span>
            </div>
          </div>
        </div>
      </div>
...
(()=>{
  ...
  async function getCameras() {
    try {
      ...
      if (cameras.length === 0) {
        cameraSelect.innerHTML = '<option value="">Aucune camera trouvee</option>';
        return;
      }
      ...
      cameraSelect.innerHTML = '<option value="">Erreur lors du chargement des cameras</option>';
    }
  }
  ...
  async function startWebcam() {
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        showInfo('Webcam demarree avec succes!');
      };

    } catch (err) {
      console.error('Error accessing webcam:', err);
      showError('Impossible d\'acceder a la webcam. Veuillez verifier les permissions et reessayer.');
    }
  }
  ...
  function updateInfo(reset = false) {
    if (reset) {
      document.getElementById('wc-infoStatus').textContent = 'Non Demarree';
    ...
    document.getElementById('wc-infoStatus').textContent = 'Active';
    ...
    document.getElementById('wc-infoFPS').textContent = `${Math.round(fps)} IPS`;
    document.getElementById('wc-infoDevice').textContent = videoTrack.label || 'Camera par Defaut';
  ...
  function downloadAll() {
    snapshots.forEach((snapshot, index) => {
      ...
      link.download = `capture-webcam-${index + 1}.png`;
      link.click();
    });
  }
  ...
  navigator.mediaDevices.getUserMedia({ video: true })
    ...
    .catch(() => {
      // Permission denied, cameras will show as "Camera 1", "Camera 2", etc.
    });
})();
</script>
