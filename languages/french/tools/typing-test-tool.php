<?php
// Typing speed tool (centralized include) - French Version
?>

<div class="typing-test-container">
  <h2>Testez votre Vitesse de Frappe</h2>
  <div class="settings">
  ...
  <p>Tapez le texte suivant le plus rapidement possible:</p>
  <div id="test-text" class="test-text">
    Chargement du texte...
  </div>
  <textarea id="typing-area" class="typing-area" placeholder="Commencez a taper ici..." disabled></textarea>
  <div class="timer-bar">
    <div id="timer-progress" class="timer-progress"></div>
  </div>
  <div class="results">
    <p>Temps restant: <span id="timer">60</span> secondes</p>
    <p>Vitesse: <span id="speed">0</span> MPM</p>
    <p>Precision: <span id="accuracy">0</span>%</p>
  </div>
  <div class="button-container">
    <button id="start-button" class="start-button">Demarrer le Test</button>
    <button id="reset-button" class="reset-button" style="display: none;">Reinitialiser</button>
  </div>
</div>

<div class="overlay" id="overlay"></div>
<div class="results-popup" id="results-popup">
  <h3>Vos Resultats de Frappe</h3>
  <p>Vitesse: <span id="final-speed">0</span> MPM</p>
  <p>Precision: <span id="final-accuracy">0</span>%</p>
  <p id="encouragement"></p>
  <button id="close-popup">Reessayer</button>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const TEXTS = {
    easy: [
      ...
      'La pratique constante ameliore votre vitesse. Maintenez un rythme stable.',
      'Tapez avec des mains calmes et concentrez-vous sur la precision.'
    ],
    medium: [
      'La vitesse de frappe s\'ameliore quand vous equilibrez precision et rythme constant.',
      'Echauffez vos doigts, maintenez une bonne posture et restez detendu.',
      'Des sessions courtes de pratique concentree construisent une memoire musculaire durable.'
    ],
    hard: [
      'La frappe precise est un melange de rythme, d\'attention et de memoire musculaire raffinee.',
      'Maintenez une cadence reguliere, corrigez les erreurs rapidement et gardez les yeux devant.',
      'La constance plutot que la vitesse gagne les tests longs; respirez, detendez-vous et concentrez-vous.'
    ]
  };
  ...
  function finishTest() {
    ...
    encouragement.textContent = Number(finalSpeed.textContent) >= 60
      ? 'Excellent rythme! Continuez a vous entrainer pour maintenir la constance.'
      : 'Bon effort! Concentrez-vous sur la precision et un rythme stable.';
