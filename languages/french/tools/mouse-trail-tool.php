<div class="game-container">
    <h1 class="game-title">Aventure Trace Emoji</h1>
    <div class="game-options">
        <h2 class="game-subtitle">Personnalisez Votre Jeu</h2>
        <div class="option-group">
            <select class="game-select" id="trailSelect" onchange="setTrail(this.value)">
                <option value="sparkles">Etincelles</option>
                ...
                <option value="lightning">Eclair</option>
                <option value="snowflake">Flocon de Neige</option>
                <option value="rocket">Fusee</option>
                <option value="rainbow">Arc-en-ciel</option>
            </select>
            <button class="game-button" id="clearTrailBtn" onclick="clearTrail()">Effacer la Trace</button>
        </div>
    </div>
    ...
    <div id="help" class="game-help">
        <h3 class="game-help-title">Aide Rapide</h3>
        <p class="game-help-text">Deplacez la souris dans la zone de jeu pour creer des traces d'emojis amusantes. Choisissez differents emojis dans le menu deroulant pour changer le type de trace. Votre score augmente en fonction du nombre de traces, de leur type et des motifs de mouvement (cercles fermes ou zigzags). Cliquez sur "Effacer la Trace" pour reinitialiser et recommencer!</p>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        trailSelect.addEventListener('change', function() {
            ...
            console.log('Trace changee en:', currentTrail);
        });
        ...
        gameArea.addEventListener('mousemove', (e) => {
            ...
            if (trails.length % 10 === 0) {
                ...
                .catch(error => console.error('Erreur de recuperation:', error));
            }
        });
        ...
        function setTrail(type) {
            ...
            console.log('Definition de la trace:', currentTrail);
            if (trails.length > 0) {
        ...
        function clearTrail() {
            console.log('Effacement des traces...');
            if (gameArea && trails) {
                trails = [];
                gameArea.innerHTML = '';
                document.getElementById('result').innerHTML = 'Trace effacee! Recommencez.';
            } else {
                console.error('Zone de jeu ou traces non trouvees!');
            }
        }
        ...
        if (clearTrailBtn) {
            ...
            console.error('Bouton Effacer la Trace non trouve!');
        }
    });
</script>
