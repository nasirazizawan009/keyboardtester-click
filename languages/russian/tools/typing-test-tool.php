<?php
// Russian Typing Speed Test Tool - Тест Скорости Печати
?>

<div class="typing-test-container">
  <h2>Проверьте Вашу Скорость Печати</h2>
  <div class="settings">
  ...
  <p>Напечатайте следующий текст как можно быстрее:</p>
  <div id="test-text" class="test-text">
    Загрузка текста...
  </div>
  <textarea id="typing-area" class="typing-area" placeholder="Начните печатать здесь..." disabled></textarea>
  <div class="timer-bar">
    <div id="timer-progress" class="timer-progress"></div>
  </div>
  <div class="results">
    <p>Осталось времени: <span id="timer">60</span> секунд</p>
    <p>Скорость: <span id="speed">0</span> СВМ</p>
    <p>Точность: <span id="accuracy">0</span>%</p>
  </div>
  <div class="button-container">
    <button id="start-button" class="start-button">Начать Тест</button>
    <button id="reset-button" class="reset-button" style="display: none;">Сбросить</button>
  </div>
</div>

<div class="overlay" id="overlay"></div>
<div class="results-popup" id="results-popup">
  <h3>Ваши Результаты Печати</h3>
  <p>Скорость: <span id="final-speed">0</span> СВМ</p>
  <p>Точность: <span id="final-accuracy">0</span>%</p>
  <p id="encouragement"></p>
  <button id="close-popup">Попробовать Снова</button>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const TEXTS = {
    easy: [
      ...
      'Постоянная практика улучшает вашу скорость. Держите ровный ритм.',
      'Печатайте спокойными руками и сосредоточьтесь на точности.'
    ],
    medium: [
      'Скорость печати улучшается когда вы балансируете точность с ровным ритмом.',
      'Разогрейте пальцы, выровняйте осанку и расслабьтесь.',
      'Короткие сфокусированные тренировки строят долговременную мышечную память.'
    ],
    hard: [
      'Точная печать это смесь ритма, внимания и отточенной мышечной памяти.',
      'Держите плавную каденцию, быстро исправляйте ошибки и смотрите вперед.',
      'Последовательность важнее скорости в длинных тестах; дышите, расслабьтесь и сосредоточьтесь.'
    ]
  };
  ...
  function finishTest() {
    ...
    encouragement.textContent = Number(finalSpeed.textContent) >= 60
      ? 'Отличный темп! Продолжайте тренироваться для поддержания стабильности.'
      : 'Хорошая работа! Сосредоточьтесь на точности и ровном ритме.';
