<?php
// Russian Webcam Test Tool - Тестер Веб-камеры
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="wc-grid">
      ...
      <div class="wc-card">
        ...
        <div class="wc-video-container" id="wc-videoContainer">
          <div class="wc-placeholder" id="wc-placeholder">
            ...
            <p style="margin: 0; font-size: 16px; font-weight: 600;">Предпросмотр Веб-камеры</p>
            <p style="margin: 8px 0 0; font-size: 13px;">Нажмите "Включить Веб-камеру" чтобы начать тест</p>
          </div>
          ...
          <div class="wc-overlay" id="wc-overlay" style="display: none;">
            <div class="wc-badge">
              ...
              <span id="wc-statusText">В Эфире</span>
            </div>
            <div class="wc-badge" id="wc-resolutionBadge">Загрузка...</div>
          </div>
        </div>

        <div class="wc-controls">
          <button id="wc-startBtn" class="wc-btn wc-btn-primary">Включить Веб-камеру</button>
          <button id="wc-stopBtn" class="wc-btn wc-btn-danger" disabled>Выключить</button>
          <button id="wc-snapshotBtn" class="wc-btn wc-btn-accent" disabled>Сделать Снимок</button>
          <button id="wc-downloadBtn" class="wc-btn wc-btn-ghost" disabled>Скачать Все</button>
        </div>

        <div id="wc-snapshotsSection" style="display: none;">
          <h4 style="margin: 20px 0 0; font-size: 14px; color: var(--muted);">Сохраненные Снимки (<span id="wc-snapshotCount">0</span>)</h4>
    ...
    <div>
      <div class="wc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Информация о Веб-камере</h3>
        <div class="wc-info-grid">
          <div class="wc-info-item">
            <span class="wc-info-label">Статус</span>
            <span class="wc-info-value" id="wc-infoStatus">Не Запущена</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Разрешение</span>
            <span class="wc-info-value" id="wc-infoResolution">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Соотношение Сторон</span>
            <span class="wc-info-value" id="wc-infoAspect">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Частота Кадров</span>
            <span class="wc-info-value" id="wc-infoFPS">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">Устройство</span>
      ...
      <div class="wc-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Настройки</h3>
        <div class="wc-settings">
          <div class="wc-setting-item">
            <label for="wc-cameraSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Выбрать Камеру</label>
          ...
          <div class="wc-setting-item">
            <label for="wc-resolutionSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">Предпочтительное Разрешение</label>
            <select id="wc-resolutionSelect" class="wc-select">
              <option value="default">Автоматически (По умолчанию)</option>
        ...
        <div style="margin-top: 20px;">
          <h4 style="margin: 0 0 12px; font-size: 14px; color: var(--muted);">Возможности</h4>
          <div class="wc-feature-list">
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Предпросмотр видео в реальном времени</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Поддержка нескольких камер</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Захват и скачивание фото</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">Определение разрешения</span>
            </div>
          </div>
        </div>
      </div>
...
(()=>{
  ...
  async function getCameras() {
    try {
      ...
      cameraSelect.innerHTML = '<option value="">Ошибка загрузки камер</option>';
    }
  }
  ...
  async function startWebcam() {
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        showInfo('Веб-камера успешно запущена!');
      };

    } catch (err) {
      console.error('Error accessing webcam:', err);
      showError('Не удалось получить доступ к веб-камере. Проверьте разрешения и попробуйте снова.');
    }
  }
  ...
  function updateInfo(reset = false) {
    if (reset) {
      document.getElementById('wc-infoStatus').textContent = 'Не Запущена';
    ...
    document.getElementById('wc-infoStatus').textContent = 'Активна';
    ...
    document.getElementById('wc-infoDevice').textContent = videoTrack.label || 'Камера по умолчанию';
  ...
  function downloadAll() {
    snapshots.forEach((snapshot, index) => {
      ...
      link.download = `снимок-вебкамеры-${index + 1}.png`;
      link.click();
    });
  }
