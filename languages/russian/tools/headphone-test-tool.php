<?php
// Russian Headphone/Speaker Test Tool - Тестер Наушников и Динамиков
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="hs-grid">
    <div>
      <div class="hs-card">
        <div id="hs-infoAlert" class="hs-alert hs-alert-info">
          🎧 <strong>Совет:</strong> Используйте наушники для лучшего тестирования стерео. Установите комфортный уровень громкости перед началом.
        </div>

        <div class="hs-visualizer" id="hs-visualizer">
          <div class="hs-placeholder" id="hs-placeholder">
            <div class="hs-placeholder-icon">🎵</div>
            <p style="margin: 0; font-size: 16px; font-weight: 600;">Визуализатор аудио</p>
            <p style="margin: 8px 0 0; font-size: 13px;">Выберите тест ниже чтобы начать</p>
          </div>
          ...
          <div class="hs-overlay" id="hs-overlay" style="display: none;">
            <div class="hs-badge">
              ...
              <span id="hs-statusText">Воспроизведение</span>
            </div>
        ...
        <div class="hs-controls">
          <button id="hs-stopBtn" class="hs-btn hs-btn-danger" disabled>Остановить</button>
        </div>
    ...
    <div>
      <div class="hs-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Быстрый тест каналов</h3>
        <div class="hs-channel-grid">
          ...
          <button class="hs-channel-btn" id="hs-rightBtn">
            <div style="font-size: 24px; margin-bottom: 8px;">➡️</div>
            <div>Правый канал</div>
          </button>
        </div>
        <button class="hs-channel-btn" id="hs-stereoBtn" style="margin-top: 12px; width: 100%;">
          <div style="font-size: 24px; margin-bottom: 8px;">🔊</div>
          <div>Оба канала (Стерео)</div>
        </button>
      </div>

      <div class="hs-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Аудио тесты</h3>
        <div class="hs-test-grid">
          <div class="hs-test-item" data-test="sweep">
            <div class="hs-test-title">🎼 Развертка частот</div>
            <div class="hs-test-desc">Развертка 20Гц - 20кГц для проверки полного диапазона</div>
          </div>
          <div class="hs-test-item" data-test="bass">
            <div class="hs-test-title">🔉 Тест басов (100Гц)</div>
            <div class="hs-test-desc">Проверка низких частот и сабвуфера</div>
          </div>
          <div class="hs-test-item" data-test="mid">
            <div class="hs-test-title">🔊 Средние частоты (1кГц)</div>
            <div class="hs-test-desc">Проверка голосов и инструментов</div>
          </div>
          <div class="hs-test-item" data-test="treble">
            <div class="hs-test-title">🔔 Тест высоких (10кГц)</div>
            <div class="hs-test-desc">Проверка высокочастотного отклика</div>
          </div>
          <div class="hs-test-item" data-test="balance">
            <div class="hs-test-title">⚖️ Тест баланса</div>
            <div class="hs-test-desc">Чередование левого и правого каналов</div>
          </div>
          <div class="hs-test-item" data-test="noise">
            <div class="hs-test-title">📻 Белый шум</div>
            <div class="hs-test-desc">Полноспектральный шум для теста динамиков</div>
          </div>
        </div>
      </div>

      <div class="hs-card" style="margin-top: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">Информация об аудио</h3>
        <div class="hs-info-grid">
          <div class="hs-info-item">
            <span class="hs-info-label">Статус</span>
            <span class="hs-info-value" id="hs-infoStatus">Готов</span>
          </div>
          <div class="hs-info-item">
            <span class="hs-info-label">Текущий тест</span>
            <span class="hs-info-value" id="hs-infoTest">Нет</span>
          </div>
          <div class="hs-info-item">
            <span class="hs-info-label">Частота</span>
            <span class="hs-info-value" id="hs-infoFrequency">-</span>
          </div>
          <div class="hs-info-item">
            <span class="hs-info-label">Канал</span>
...
(()=>{
  ...
  function playTone(frequency, panValue = 0, channelName = 'Стерео') {
    stopAudio();
    initAudioContext();
    ...
    updateInfo(false, `Тон ${frequency}Гц`, frequency, channelName);
  }
  ...
  function playNoise() {
    ...
    updateInfo(false, 'Белый шум', 'Полный спектр', 'Стерео');
  }
  ...
  function playSweep() {
    ...
    updateInfo(false, 'Развертка частот', '20Гц - 20кГц', 'Стерео');
  }
  ...
  function playBalance() {
    ...
    const balanceInterval = setInterval(() => {
      ...
      const channelName = panValue === -1 ? 'Левый' : 'Правый';
    ...
    updateInfo(false, 'Тест баланса', '440Гц', 'Чередование');
  }
    ...
    function draw() {
      ...
      if (isSweep && oscillator && sweepStartTime) {
        ...
        if (currentFreq <= 20000) {
          frequencyBadge.textContent = `${currentFreq}Гц`;
          infoFrequency.textContent = `${currentFreq}Гц`;
        }
      }
    }
  ...
  function updateInfo(reset = false, testName = '', frequency = '', channel = '') {
    if (reset) {
      infoStatus.textContent = 'Готов';
      infoStatus.style.color = 'var(--muted)';
      infoTest.textContent = 'Нет';
    ...
    infoStatus.textContent = 'Воспроизведение';
    ...
    item.addEventListener('click', function() {
      ...
      switch(testType) {
        case 'sweep':
          ...
          break;
        case 'bass':
          playTone(100, 0, 'Стерео');
          break;
        case 'mid':
          playTone(1000, 0, 'Стерео');
          break;
        case 'treble':
          playTone(10000, 0, 'Стерео');
          break;
        case 'balance':
          playBalance();
          break;
