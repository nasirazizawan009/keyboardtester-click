        ...
        <div class="keyboard-layout jis-layout">
            <div class="keyboard-section main-keyboard">
                ...
                <div class="key-row">
                    ...
                    <button class="key key-enter-jis" data-key="Enter"><span>Enter</span></button>
                </div>
...
.key-enter-jis { width: 66px; height: 48px; }
...
@media (max-width: 992px) {
    ...
    .key-enter-jis { width: 55px; height: 40px; }
...
@media (max-width: 768px) {
    ...
    .key-enter-jis { width: 50px; height: 36px; }
...
@media (max-width: 480px) {
    ...
    .key-enter-jis { width: 44px; height: 32px; }
...
(function() {
    ...
    const catProgress = (catProgressRoot && window.KeyboardCatProgress)
        ? new window.KeyboardCatProgress(catProgressRoot, { desktopTotalKeys: TOTAL_KEYS, mobileTotalKeys: 42, locale: 'ja' })
        : null;
...
function handleKeyDown(e) {
    ...
    function reset() { state.keyPressCount = {}; state.uniqueKeys.clear(); state.totalPresses = 0; state.sessionStart = null; state.latencyData = []; state.lastKeyTime = 0; if (state.sessionTimer) { clearInterval(state.sessionTimer); state.sessionTimer = null; } if ($('key-history')) $('key-history').value = ''; if ($('total-keys')) $('total-keys').textContent = '0'; if ($('session-time')) $('session-time').textContent = '00:00'; if ($('keys-tested')) $('keys-tested').textContent = `0/${TOTAL_KEYS}`; if ($('most-pressed')) $('most-pressed').textContent = 'なし'; if ($('current-latency')) $('current-latency').textContent = '-- ms'; if ($('avg-latency')) $('avg-latency').textContent = '--'; if ($('min-latency')) $('min-latency').textContent = '--'; if ($('max-latency')) $('max-latency').textContent = '--'; $$('.key').forEach(k => { k.classList.remove('pressed', 'active-1', 'active-2', 'active-3', 'active-4', 'active-5'); const counter = k.querySelector('.key-counter'); if (counter) counter.remove(); }); $$('.indicator').forEach(i => i.classList.remove('active')); resetCatProgress(); }
...
if (!isset($GLOBALS['__kbt_adv_loaded'])) {
    ...
    echo "<script>window.KBT_LOCALE = 'ja';</script>";
