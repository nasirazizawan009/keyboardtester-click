<div class="game-container">
    <h1 class="game-title">絵文字トレイルアドベンチャー</h1>
    <div class="game-options">
        <h2 class="game-subtitle">ゲームをカスタマイズ</h2>
        <div class="option-group">
            <select class="game-select" id="trailSelect" onchange="setTrail(this.value)">
                ...
                <option value="snowflake">❄️ 雪の結晶</option>
                <option value="rocket">🚀 ロケット</option>
                <option value="rainbow">🌈 虹</option>
            </select>
            <button class="game-button" id="clearTrailBtn" onclick="clearTrail()">トレイルをクリア</button>
        </div>
    </div>
    ...
    <div id="help" class="game-help">
        <h3 class="game-help-title">クイックヘルプ</h3>
        <p class="game-help-text">ゲームエリア内でマウスを動かして楽しい絵文字トレイルを作成しましょう。ドロップダウンメニューから異なる絵文字を選んでトレイルの種類を変更できます。スコアはトレイルの数、種類、動きのパターン（閉じた円や激しいジグザグ）に基づいて増加します。「トレイルをクリア」をクリックしてリセットし、最初からやり直しましょう！</p>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function clearTrail() {
            ...
            if (gameArea && trails) {
                trails = [];
                gameArea.innerHTML = '';
                document.getElementById('result').innerHTML = 'トレイルをクリアしました！最初からやり直しましょう。';
            } else {
