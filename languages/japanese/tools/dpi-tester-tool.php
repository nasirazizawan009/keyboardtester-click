...
// マウスDPIテスター v2.0 — 日本語
$dpi_strings = [
  'tab_test'=>'DPIテスト','tab_edpi'=>'eDPI計算機','tab_conv'=>'感度変換',
  'setup'=>'1 — セットアップ','lbl_dist'=>'マウスを動かす実際の距離',
  'unit_cm'=>'cm','unit_in'=>'インチ','lbl_exp'=>'設定DPI',
  'exp_hint'=>'(省略可 — ずれ率と精度評価を有効化)',
  'lbl_axis'=>'測定軸の制限','axis_both'=>'任意方向（推奨）',
  'axis_x'=>'水平のみ（X軸）','axis_y'=>'垂直のみ（Y軸）',
  'btn_start'=>'▶ 計測開始','btn_reset'=>'リセット',
  'tip'=>'<strong>ヒント：</strong>マウスパッドに定規を当てて始点と終点に印を付け、マウスボタンを押しながら計測エリアをドラッグしてください。',
  'track_lbl'=>'DPIトラッキングエリア','track_title'=>'2 — 計測と結果',
  'track_idle'=>'計測開始を押してからここでドラッグ',
  'track_ready'=>'ボタンを押しながら計測距離をドラッグ',
  'stat_px'=>'移動ピクセル数','stat_dpi'=>'推定DPI','stat_dev'=>'ずれ率','stat_acc'=>'精度',
  'runs_lbl'=>'計測履歴（ドラッグで追加）','run_n'=>'試行','run_avg'=>'平均:',
  'run_more'=>' / %d回 — 再度ドラッグ ↔','run_done'=>'全試行完了 — 上の平均を確認',
  'privacy'=>'🔒 このテストはブラウザ内で完結します。データは収集されません。',
  'edpi_setup'=>'あなたの設定','lbl_dpi'=>'マウスDPI','lbl_sens'=>'ゲーム内感度',
  'lbl_game'=>'ゲーム（比較用）','game_any'=>'— 任意 / カスタム —',
  'edpi_lbl'=>'eDPI — 実効DPI','edpi_formula'=>'<strong>計算式：</strong> eDPI = マウスDPI × ゲーム内感度',
  'pro_title'=>'プロ比較','pro_intro'=>'あなたのeDPIが表内でハイライトされます。',
  'pro_h1'=>'eDPI範囲','pro_h2'=>'レベル','pro_h3'=>'代表的ユーザー',
  'conv_curr'=>'現在の設定','lbl_old_dpi'=>'現在のDPI',
  'lbl_old_s'=>'現在のゲーム内感度','conv_target'=>'目標設定',
  'lbl_new_dpi'=>'新しいDPI（マウス変更後）',
  'conv_sub'=>'新しい感度 — 同じ筋肉記憶、異なるDPI',
  'conv_how'=>'仕組み',
  'conv_exp1'=>'変換機は<strong>eDPIを一定</strong>に保つので、同じ物理的な動きが常に同じゲーム内視点移動を生みます。',
  'conv_formula'=>'新感度 = (旧DPI × 旧感度) / 新DPI',
  'conv_exp2'=>'<strong>例：</strong> 400DPI 感度2.0 → 800DPI 感度1.0。両方eDPI=800。',
  'log_title'=>'セッションログ',
  'log_dl'=>'&#11015; ログをダウンロード (.txt)',
  'log_empty'=>'まだ計測がありません。DPIテストを実行してください。',
];
?>
...
<div class="kbt-tool dpi-v2" id="dpi-tool">
  ...
  <div class="dpi-panel active" id="dpi-p-test" role="tabpanel">
    <div class="dpi-mode-row"><div class="dpi-mode-toggle"><button class="dpi-mode-btn active" data-mode="basic">基本</button><button class="dpi-mode-btn" data-mode="advanced">詳細</button></div></div>
    <div class="dpi-grid">
      <div class="dpi-card">
        ...
        <div class="dpi-field"><label for="dpi-expected"><?php echo $dpi_strings['lbl_exp']; ?> <small><?php echo $dpi_strings['exp_hint']; ?></small></label><input class="dpi-input full" id="dpi-expected" type="number" min="100" max="32000" step="100" placeholder="例: 800"></div>
  ...
  <div class="dpi-panel" id="dpi-p-edpi" role="tabpanel">
    <div class="dpi-grid">
      ...
      <div class="dpi-card">
        ...
        <tbody id="edpi-tbody">
          <tr data-min="0"    data-max="400">   <td>200–400</td>  <td>超低感度</td>    <td>FPSプロ (S1mple, sh1ro)</td></tr>
          <tr data-min="400"  data-max="800">   <td>400–800</td>  <td>低感度</td>      <td>競技FPS / Valorantプロ</td></tr>
          <tr data-min="800"  data-max="1600">  <td>800–1600</td> <td>中感度</td>      <td>一般ゲーマー、カジュアルFPS</td></tr>
          <tr data-min="1600" data-max="3200">  <td>1600–3200</td><td>高感度</td>      <td>MOBA / RTS</td></tr>
          <tr data-min="3200" data-max="999999"><td>3200+</td>    <td>超高感度</td>    <td>オフィス、デザイン</td></tr>
        </tbody></table>
      </div>
    </div>
    <p class="dpi-privacy"><?php echo $dpi_strings['privacy']; ?></p>
    ...
    <div class="dpi-grid">
      <div class="dpi-card">
        ...
        <div class="dpi-conv-result">
          <p class="dpi-conv-formula" id="conv-formula">新感度 = (400 &times; 2.0) / 800</p>
...
function accuracy(p){if(p<=2)return{label:'\u512a \u2713',cls:'s-good'};if(p<=5)return{label:'\u826f \u2713',cls:'s-good'};if(p<=10)return{label:'\u53ef',cls:'s-warn'};return{label:'\u4e0d\u53ef',cls:'s-bad'};}
...
function updConv(){var od=parseFloat(cOd.value)||0,os=parseFloat(cOs.value)||0,nd=parseFloat(cNd.value)||0;if(od&&os&&nd){cFm.innerHTML='\u65b0\u611f\u5ea6 = ('+od+' &times; '+os+') / '+nd;cVl.textContent=((od*os)/nd).toFixed(4);}else{cVl.textContent='--';}}
