<?php
// Japanese Mouse Tester Tool - マウステスター
?>
<div class="mouse-tester">
  <canvas id="mouse-canvas" width="400" height="250" role="button" aria-label="マウステストエリア、任意のボタンをクリックまたはスクロールしてください"></canvas>

  <div class="results">
    <div class="result-box" id="left-click-count" aria-live="polite">左クリック: 0</div>
    <div class="result-box" id="middle-click-count" aria-live="polite">中クリック: 0</div>
    <div class="result-box" id="right-click-count" aria-live="polite">右クリック: 0</div>
    <div class="result-box" id="scroll-count" aria-live="polite">スクロール: 0</div>
  </div>

  <div class="status">
    <div class="status-box" id="left-click-status">左: 解放 (最後: なし)</div>
    <div class="status-box" id="middle-click-status">中: 解放 (最後: なし)</div>
    <div class="status-box" id="right-click-status">右: 解放 (最後: なし)</div>
    <div class="status-box" id="scroll-status">スクロール: なし (最後: なし)</div>
  </div>

  <button class="reset-button" id="reset-button" aria-label="すべてのカウンターとステータスをリセット">リセット</button>
</div>
...
<script>
  ...
  const buttonStates = {
    left: { status: '解放', lastPress: 'なし' },
    middle: { status: '解放', lastPress: 'なし' },
    right: { status: '解放', lastPress: 'なし' },
    scroll: { status: 'なし', lastPress: 'なし' }
  };
  ...
  function updateStatus(button, status, timestamp) {
    ...
    if (timestamp) buttonStates[button].lastPress = new Date().toLocaleTimeString('ja-JP');
    const buttonNames = { left: '左', middle: '中', right: '右', scroll: 'スクロール' };
    const displayText = button === 'scroll' ?
      `スクロール: ${buttonStates[button].status} (最後: ${buttonStates[button].lastPress})` :
      `${buttonNames[button]}: ${buttonStates[button].status} (最後: ${buttonStates[button].lastPress})`;
    switch (button) {
      case 'left':
        leftClickStatus.textContent = displayText;
        leftClickStatus.classList.toggle('left-click-pressed', status === '押下');
        break;
      case 'middle':
        middleClickStatus.textContent = displayText;
        middleClickStatus.classList.toggle('middle-click-pressed', status === '押下');
        break;
      case 'right':
        rightClickStatus.textContent = displayText;
        rightClickStatus.classList.toggle('right-click-pressed', status === '押下');
        break;
      case 'scroll':
        scrollStatus.textContent = displayText;
        scrollStatus.classList.toggle('scroll-pressed', status !== 'なし');
        break;
    }
  }
  ...
  canvas.addEventListener('mousedown', (event) => {
    ...
    if (buttonType) {
      ...
      switch (buttonType) {
        case 'left':
          leftClickCount++;
          leftClickDisplay.textContent = `左クリック: ${leftClickCount}`;
          leftClickDisplay.classList.add('left-click-active');
          updateStatus('left', '押下', true);
          break;
        case 'middle':
          middleClickCount++;
          middleClickDisplay.textContent = `中クリック: ${middleClickCount}`;
          middleClickDisplay.classList.add('middle-click-active');
          updateStatus('middle', '押下', true);
          break;
        case 'right':
          rightClickCount++;
          rightClickDisplay.textContent = `右クリック: ${rightClickCount}`;
          rightClickDisplay.classList.add('right-click-active');
          updateStatus('right', '押下', true);
          break;
      }
    }
  });
  ...
  canvas.addEventListener('mouseup', (event) => {
    event.preventDefault();
    switch (event.button) {
      case 0: updateStatus('left', '解放'); break;
      case 1: updateStatus('middle', '解放'); break;
      case 2: updateStatus('right', '解放'); break;
    }
  });
  ...
  resetButton.addEventListener('click', () => {
    ...
    scrollOffset = 0;
    lastScrollDirection = null;
    buttonStates.left = { status: '解放', lastPress: 'なし' };
    buttonStates.middle = { status: '解放', lastPress: 'なし' };
    buttonStates.right = { status: '解放', lastPress: 'なし' };
    buttonStates.scroll = { status: 'なし', lastPress: 'なし' };
    leftClickDisplay.textContent = '左クリック: 0';
    middleClickDisplay.textContent = '中クリック: 0';
    rightClickDisplay.textContent = '右クリック: 0';
    scrollDisplay.textContent = 'スクロール: 0';
    leftClickStatus.textContent = '左: 解放 (最後: なし)';
    middleClickStatus.textContent = '中: 解放 (最後: なし)';
    rightClickStatus.textContent = '右: 解放 (最後: なし)';
    scrollStatus.textContent = 'スクロール: なし (最後: なし)';
