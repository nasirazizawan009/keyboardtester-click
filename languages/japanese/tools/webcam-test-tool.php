<?php
// Japanese Webcam Test Tool - ウェブカメラテスター
?>
<style>
  :root {
    --bg: #f6f8fc;
  ...
  <div class="wc-grid">
      ...
      <div class="wc-card">
        ...
        <div class="wc-video-container" id="wc-videoContainer">
          <div class="wc-placeholder" id="wc-placeholder">
            ...
            <p style="margin: 0; font-size: 16px; font-weight: 600;">ウェブカメラプレビュー</p>
            <p style="margin: 8px 0 0; font-size: 13px;">「ウェブカメラ開始」をクリックしてテストを開始</p>
          </div>
          ...
          <div class="wc-overlay" id="wc-overlay" style="display: none;">
            <div class="wc-badge">
              ...
              <span id="wc-statusText">ライブ</span>
            </div>
            <div class="wc-badge" id="wc-resolutionBadge">読み込み中...</div>
          </div>
        </div>

        <div class="wc-controls">
          <button id="wc-startBtn" class="wc-btn wc-btn-primary">ウェブカメラ開始</button>
          <button id="wc-stopBtn" class="wc-btn wc-btn-danger" disabled>ウェブカメラ停止</button>
          <button id="wc-snapshotBtn" class="wc-btn wc-btn-accent" disabled>スナップショット</button>
          <button id="wc-downloadBtn" class="wc-btn wc-btn-ghost" disabled>すべてダウンロード</button>
        </div>

        <div id="wc-snapshotsSection" style="display: none;">
          <h4 style="margin: 20px 0 0; font-size: 14px; color: var(--muted);">保存されたスナップショット (<span id="wc-snapshotCount">0</span>)</h4>
    ...
    <div>
      <div class="wc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 16px; font-size: 16px;">ウェブカメラ情報</h3>
        <div class="wc-info-grid">
          <div class="wc-info-item">
            <span class="wc-info-label">ステータス</span>
            <span class="wc-info-value" id="wc-infoStatus">未開始</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">解像度</span>
            <span class="wc-info-value" id="wc-infoResolution">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">アスペクト比</span>
            <span class="wc-info-value" id="wc-infoAspect">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">フレームレート</span>
            <span class="wc-info-value" id="wc-infoFPS">-</span>
          </div>
          <div class="wc-info-item">
            <span class="wc-info-label">デバイス</span>
      ...
      <div class="wc-card">
        <h3 style="margin: 0 0 16px; font-size: 16px;">設定</h3>
        <div class="wc-settings">
          <div class="wc-setting-item">
            <label for="wc-cameraSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">カメラ選択</label>
          ...
          <div class="wc-setting-item">
            <label for="wc-resolutionSelect" style="font-size: 13px; font-weight: 600; color: var(--muted);">希望解像度</label>
            <select id="wc-resolutionSelect" class="wc-select">
              <option value="default">自動（デフォルト）</option>
              <option value="4k">4K (3840x2160)</option>
              <option value="1080p">フルHD (1920x1080)</option>
        ...
        <div style="margin-top: 20px;">
          <h4 style="margin: 0 0 12px; font-size: 14px; color: var(--muted);">機能</h4>
          <div class="wc-feature-list">
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">リアルタイムビデオプレビュー</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">複数カメラ対応</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">写真キャプチャ＆ダウンロード</span>
            </div>
            <div class="wc-feature-item">
              <span class="wc-feature-icon">✓</span>
              <span class="wc-feature-text">解像度検出</span>
            </div>
          </div>
        </div>
      </div>
...
(()=>{
  ...
  async function getCameras() {
    try {
      ...
      cameraSelect.innerHTML = '<option value="">カメラの読み込みエラー</option>';
    }
  }
  ...
  async function startWebcam() {
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        showInfo('ウェブカメラが正常に開始されました！');
      };

    } catch (err) {
      console.error('Error accessing webcam:', err);
      showError('ウェブカメラにアクセスできませんでした。権限を確認して再試行してください。');
    }
  }
  ...
  function updateInfo(reset = false) {
    if (reset) {
      document.getElementById('wc-infoStatus').textContent = '未開始';
    ...
    document.getElementById('wc-infoStatus').textContent = 'アクティブ';
    ...
    document.getElementById('wc-infoDevice').textContent = videoTrack.label || 'デフォルトカメラ';
